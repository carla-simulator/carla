...
def start_tracis_synchronization(args, env, sumo_files):
    ...
    if args.main_mobility_handler == "carla":
        return Popen(f"python ./synch/run_tracis_synchronization.py {args.env_file_path} --main_sumo_host_port 127.0.0.1:{args.carla_sumo_port} --other_sumo_host_ports {env['vagrant_ip']}:{args.veins_sumo_port} --sumo_order {2} --time_to_start {args.time_to_start} --time_to_finish {args.time_to_finish} {dev_null}", shell=True)
    else:
        return Popen(f"python ./synch/run_tracis_synchronization.py {args.env_file_path} --main_sumo_host_port {env['vagrant_ip']}:{args.veins_sumo_port} --other_sumo_host_ports 127.0.0.1:{args.carla_sumo_port} --sumo_order {2} --time_to_start {args.time_to_start} --time_to_finish {args.time_to_finish} {dev_null}", shell=True)
