...
def main(args, env):
    ...
    try:
        ...
        carla_sumo_proc = Popen(f"{args.sumocmd} -c {sumo_files['sumocfg']} --begin {args.sumo_begin_time} --end {args.sumo_end_time} --step-length {args.sumo_step_length} --remote-port {args.carla_sumo_port} --num-clients 2 > /dev/null 2>&1", shell=True)
        ...
        veins_sumo_proc = Popen(f"{args.sumocmd} -c {sumo_files['sumocfg']} --begin {args.sumo_begin_time} --end {args.sumo_end_time} --step-length {args.sumo_step_length} --remote-port {args.veins_sumo_port} --num-clients 2 > /dev/null 2>&1", shell=True)
        ...
        copy_local_file_to_vagrant(sumo_files["sumocfg"], f"{env['sumo_files_name_in_veins']}.sumocfg", env['veins_ini_dir_in_vagrant'], args.veins_vagrant_path)
