...
# Copyright (c) 2020 Comremoteer Vision Center (CVC) at the Universitat Autonoma de
# Barcelona (UAB).
#
...
from util.ray.perceived_objects import (
...
from util.ray.sensor_data import (
    SensorDataHandler,
...
class CAV:
    def __init__(self, sumo_actor_id, carla_actor_id, carla_sim, sumo_sim, carla_init_time, sim_synchronization):
        ...
        self.sensor_data_handler = SensorDataHandler.remote()
        self.perceived_objects_handler = PerceivedObjectsHandler.remote()

        self.CAMs_handler = CAMsHandlerWithNoSend.remote(DATA_SERVER_HOST, DATA_SERVER_PORT, DATA_DIR)
        self.CPMs_handler = CPMsHandlerWithNoSend.remote(DATA_SERVER_HOST, DATA_SERVER_PORT, DATA_DIR)
    ...
    def new_perceived_objects_with_pseudonym(self):
        new_perceived_objects = self.sensor_data_handler.perceived_objects.remote(self.sumo_elapsed_seconds(), Constants.VALID_TIME_DELTA)

        return self.perceived_objects_handler.save_all.remote(new_perceived_objects)


    def sensor_data_num(self):
        return ray.get(self.sensor_data_handler.data_num.remote())
    ...
    def tick(self):
        ...
        # ----- send CPM -----
        perceived_object_container = self.CPMs_handler.new_perceived_object_container.remote(self.new_perceived_objects_with_pseudonym())
        t1 = time.time()
        cpm_send_pid = self.CPMs_handler.send_with_info.remote(
        ...
        ray_procs.append(cpm_send_pid)

        t2 = time.time()

        # ----- send CAM -----
        # print(f"sumo_id: {self.sumo_actor_id}")
        if ray.get(self.CAMs_handler.is_generate.remote(self.sumo_elapsed_seconds(), self.location(), self.speed(), self.yaw())):
            cam_send_pid = self.CAMs_handler.send.remote(self.sumo_actor_id, CAM(
                timestamp=self.sumo_elapsed_seconds(),
                ITS_PDU_Header=self.__tmp_data(),
...
class CAVWithRaderSensors(CAV):
    def __init__(self, sumo_actor_id, carla_actor_id, carla_sim, sumo_sim, carla_init_time, sim_synchronization):
        ...
        self.sensor_data_handler = RadarSensorDataHandler.remote()
    ...
    def receive_sensor_data(self, data):
        for d in data:
            ...
            try:
                ray.get(self.sensor_data_handler.save.remote(RadarSensorData(
...
class CAVWithObstacleSensors(CAV):
    def __init__(self, sumo_actor_id, carla_actor_id, carla_sim, sumo_sim, carla_init_time, sim_synchronization):
        ...
        super().__init__(sumo_actor_id, carla_actor_id, carla_sim, sumo_sim, carla_init_time, sim_synchronization)
        self.sensor_data_handler = ObstacleSensorDataHandler.remote()
    ...
    def receive_sensor_data(self, data):
        ...
        try:
            ray.get(self.sensor_data_handler.save.remote(ObstacleSensorData(
                self.sumo_elapsed_seconds(),
...
class SimulationSynchronization(object):
    ...
    def tick(self):
        ...
        # ----- ray based -----
        ray_procs = [cav.tick() for cav in self.sumoid2cav.values()]
        for ps in ray_procs:
            ray.get(ps)
    ...
    def get_cams_handler_by_standard(self, std):
        if std == "etsi":
            return CAMsHandlerWithEtsi.remote(DATA_SERVER_HOST, DATA_SERVER_PORT, DATA_DIR)
        elif std == "periodic":
            return CAMsHandlerWithInterval.remote(DATA_SERVER_HOST, DATA_SERVER_PORT, DATA_DIR)
        elif std == "no":
            return CAMsHandlerWithNoSend.remote(DATA_SERVER_HOST, DATA_SERVER_PORT, DATA_DIR)
        else:
            raise Exception(f"No such CAM standard: {std}")

    def get_cpms_handler_by_standard(self, std):
        if std == "etsi":
            return CPMsHandlerWithEtsi.remote(DATA_SERVER_HOST, DATA_SERVER_PORT, DATA_DIR)
        elif std == "periodic":
            return CPMsHandlerWithInterval.remote(DATA_SERVER_HOST, DATA_SERVER_PORT, DATA_DIR)
        elif std == "no":
            return CPMsHandlerWithNoSend.remote(DATA_SERVER_HOST, DATA_SERVER_PORT, DATA_DIR)
        else:
