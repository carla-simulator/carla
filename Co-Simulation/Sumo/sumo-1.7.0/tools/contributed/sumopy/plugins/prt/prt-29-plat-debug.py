...
class PrtStops(am.ArrayObjman):
    ...
    def process_step(self, process):
        ...
        for id_stop, id_edge_sumo, ids_veh_sumo_prev, ids_person_sumo_prev in\
            ...
            if True:  # len(self.ids_vehs_alight_allocated[id_stop])==0:
                ...
                for id_berth_alight, id_veh in zip(
                    ...
                    if id_veh >= 0:  # is there a waiting vehicle
                        ...
                        print '    check alloc veh in front', self.ids_vehs[id_stop][:
                                                                                     ind], self.ids_vehs_alight_allocated[id_stop]
                ...
                if n_veh_alloc > 0:
                    ...
                    print '  found %d veh at id_stop=%d to berth alloc with board alloc index %d' % (
                        n_veh_alloc, id_stop, self.inds_berth_board_allocated[id_stop])
            ...
            print '  check for reset of alight allocation index', self.inds_berth_alight_allocated[id_stop], len(
                self.ids_vehs_alight_allocated[id_stop]) == 0, np.all(berths.states[ids_berth_alight] == BERTHSTATES['free'])

            if (len(self.ids_vehs_alight_allocated[id_stop]) == 0) & np.all(berths.states[ids_berth_alight] == BERTHSTATES['free']):

                print '    reset inds_berth_alight_allocated', self.inds_berth_alight_allocated[id_stop], '->', len(
                    self.ids_berth_alight[id_stop])
                self.inds_berth_alight_allocated[id_stop] = len(self.ids_berth_alight[id_stop])
            ...
            print '  check for reset of board berth', (self.inds_berth_board_allocated[id_stop] == 0), (len(
                self.ids_vehs_board_allocated[id_stop]) == 0), np.all(berths.states[ids_berth_board] == BERTHSTATES['free'])
...
class PrtVehicles(am.ArrayObjman):
    ...
    def concatenate(self, id_veh, id_veh_pre):
        ...
        if 0:
            traci.vehicle.setSpeedFactor(id_veh_sumo, 2.0)  # +random.uniform(-0.01,0.01)
            traci.vehicle.setImperfection(id_veh_sumo, 0.0)
            # traci.vehicle.setDecel(id_veh_sumo,3.5)
            traci.vehicle.setAccel(id_veh_sumo, 3.5)
            traci.vehicle.setMinGap(id_veh_sumo, 0.01)
            traci.vehicle.setTau(id_veh_sumo, 0.4)
