...
class PrtStops(am.ArrayObjman):
    ...
    def prepare_sim(self, process):
        ...
        self.stoplines[ids] = net.edges.lengths[ids_stopedge]-5.0
    ...
    def process_step(self, process):
        ...
        for id_stop, id_edge_sumo, ids_veh_sumo_prev, ids_person_sumo_prev in\
            ...
            for time_start, id_veh, id_stop_target in self.ids_vehs_prog[id_stop]:
    ...
    def program_leadveh(self, id_stop, id_veh, id_stop_target, time_start):
        print 'program_leadveh', id_stop, id_veh, id_stop_target
        ...
        if id_veh == self.ids_veh_lead[id_stop]:
            ...
            if self.parent.prtvehicles.states[id_veh] == VEHICLESTATES['waiting']:
                self.init_trip_empty(id_stop, id_berth, id_veh, is_ask_vehman=False)

            self.ids_veh_lead[id_stop] = -1

            self.ids_vehs_prog[id_stop].append((time_start, id_veh, id_stop_target))
            ...
            print '  new lead veh', self.ids_veh_lead[id_stop]
            return True
    ...
    def init_trip_occupied(self, id_stop, id_berth, id_veh, simtime):
        ...
        print 'init_trip_occupied', id_stop, id_berth, id_veh
    ...
    def exit(self, id_stop, id_veh):
        ...
        #ind_veh = -1
        i = 0
        for time_start, id_veh_prog, id_stop_target in self.ids_vehs_prog[id_stop]:
...
class PrtVehicles(am.ArrayObjman):
    ...
    def init_trip_empty(self, id_veh, id_edge_sumo, stopline):
        print 'init_trip_empty', self.get_id_sumo(id_veh), id_edge_sumo, stopline
...
class VehicleMan(am.ArrayObjman):
    ...
    def _init_attributes(self):
        ...
        self.add(cm.AttrConf('time_update_flows', 30.0,
                             groupnames=['parameters'],
    ...
    def prepare_sim(self, process):
        ...
        self.n_horizon = self.time_horizon.get_value()/self.time_update_flows.get_value()
        self.inflows_est = np.zeros((n_stoparray, self.n_horizon), dtype=np.int32)
    ...
    def process_step(self, process):
        ...
        inds_valid = ids_veh_lead >= 0

        self.assign_occupied_leadvehs(ids_veh_lead[inds_valid], process)
        self.assign_empty_leadvehs(ids_veh_lead[inds_valid], process)

    # def start_trip(self, id_veh, id_stop):
    #    if self.are_emptytrips[id_veh]:
    #        id_stop_target = self.get_stop_emptytrip(id_stop)
    #        self.ids_stop_target[id_veh] = id_stop_target
    #    else:
    #        id_stop_target = self.ids_stop_target[id_veh]
    #
    ...
    def assign_occupied_leadvehs(self, ids_veh_lead, process):
        ...
        print 'assign_occupied_leadvehs'
        for id_veh_lead, state, id_stop_current, id_stop_target, time_order\
                in zip(
            ...
            print '  check veh', id_veh_lead, state, 'id_stop_current', id_stop_current, 'id_stop_target', id_stop_target, 'time_order', time_order
            #VEHICLESTATES = {'init':0,'waiting':1,'boarding':2,'alighting':3,'emptytrip':4,'occupiedtrip':5,'forewarding':6}
            # if state == VEHICLESTATES['occupiedtrip']:
            stops.program_leadveh(id_stop_current, id_veh_lead, id_stop_target, time_order)
            self.numbers_veh_arr[id_stop_target] += 1
    ...
    def assign_empty_leadvehs(self, ids_veh_lead, process):
        ...
        print 'assign_empty_leadvehs'

        for id_veh_lead, state, id_stop_current, time_order\
                in zip(
                    ids_veh_lead[inds_valid],
                    vehicles.states[ids_veh_lead[inds_valid]],
                    self.ids_stop_current[ids_veh_lead[inds_valid]],
    ...
    def init_trip_empty(self, id_veh, id_stop, time_order):
        # print 'init_trip_empty id_veh,id_stop',id_veh,id_stop
        # search closest stop
