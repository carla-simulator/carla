...
VEHICLESTATES = {'init': 0, 'wait': 1, 'boarding': 2, 'alighting': 3, 'emptytrip': 4, 'occupiedtrip': 5, }
...
class PrtStops(am.ArrayObjman):
        ...
        for id_prtstop, id_edge_sumo, ids_veh_sumo_prev in\
            ...
            for id_veh in self.ids_vehs_alight_allocated[id_prtstop]:
                ...
                if traci.vehicle.isStopped(vehicles.get_id_sumo(id_veh)):
                    ...
                    berths.ids_veh[vehicles.ids_berth[id_veh]] = id_veh
            ...
            ids_berth_board = self.ids_berth_alight[id_prtstop][::-1]
            for id_berth_alight, id_veh, id_berth_board in zip(
                ...
                if id_veh >= 0:  # is there is a waiting vehicle
                    if vehicles.is_completed_alight(id_veh):
                        print '     send vehicle to id_berth_board', id_berth_board
            ...
            if len(self.ids_vehs_alight_allocated[id_prtstop]) == 0:
                if np.all(berths.state[ids_berth_alight] == BERTHSTATES['free']):
    ...
    def allocate_alight(self, id_stop):
        ...
        if ind_berth == 0:
            ...
            # return
            #berths = self.get_berths()
            # print '  stop at id_berth,stoppositions',id_berth,berths.stoppositions[id_berth]
            # traci.vehicle.setStop(  id_veh_sumo,
            #                self.ids_stop_to_ids_edge_sumo[id_stop],
            #                pos=berths.stoppositions[id_berth],
            #                #flags= 4,
            #                laneIndex= 1,
            #                )
            # if not is_new:
            #    if id_veh_sumo in  self.ids_vehs_toallocate[id_stop]:
            #        self.ids_vehs_toallocate[id_stop].remove(id_veh_sumo)
...
class PrtVehicles(am.ArrayObjman):
    ...
    def control_stop(self, id_veh, id_stop, id_berth,
                     id_edge_sumo=None,
                     position=None,
                     is_boarding=False,
                     ):
        ...
        # attentoion: trigger and next stop??
        if is_boarding:
            flags = 2
        else:
            flags = 0
