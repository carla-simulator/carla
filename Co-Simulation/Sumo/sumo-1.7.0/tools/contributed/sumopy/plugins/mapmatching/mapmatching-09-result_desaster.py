...
# @file    mapmatching-09-result_desaster.py
...
class BirgilMatcher(Process):
    ...
    def do(self):
        ...
        tripresults = self._results.tripresults
        ...
        for id_mode in set(ids_mode):
            ...
            costs[id_mode] = edges.get_distances(id_mode=id_mode,
                                                 is_check_lanes=False
                                                 )
        ...
        for id_trip, ids_point, id_mode in zip(ids_trip, trips.ids_points[ids_trip], ids_mode):
            ...
            tripresults.set_matched_route(id_trip, route,
                                          length_route,
                                          length_route_mixed,
...
class GpsTrips(Trips):
    ...
    def route_shortest(self, is_check_lanes=True):
        """
...
class Matchresults(cm.BaseObjman):
    ...
    def _init_attributes(self):
        ...
        self.tripresults = attrsman.add(cm.ObjConf(Tripresults('tripresults',
                                                               self, mapmatching.trips),
...
class Tripresults(am.ArrayObjman):
    def __init__(self, ident, parent, trips,
                 is_add_default=True,
                 name='Trip results',
                 info='Table with results for each matched trip.',
                 **kwargs):
    ...
    def _init_attributes(self):
        self.add(cm.ObjConf(Routes('routes', self,
        ...
        self.add_col(am.ArrayConf('lengths_route_matched', default=-1.0,
                                  ...
                                  symbol='L_{M}',
                                  unit='m',
                                  info='Length of the matched part of the GPS trace. Note the only a fraction of the GPS trace may be within the given network.',
                                  ))
    ...
    def get_ids_route(self):
        # TODO: here we could append direct routes
        print 'get_ids_route'
        ids_route_matched = self.ids_route_matched[self.get_ids()]
    ...
    def set_matched_route(self, id_trip, route,
        ...
        ids_tripresult = self.get_ids()
        ids_trip = self.ids_trip[ids_tripresult]
        inds = np.flatnonzero(ids_trip == id_trip)
        if len(inds) == 0:
            # no previous result for id_trip
            id_tripresult = self.add_row()
        else:
            # overwrite previous result
            id_tripresult = ids_tripresult[inds[0]]

        if len(route) > 0:
            id_route = self.ids_route_matched[id_tripresult]
            if id_route >= 0:
                ...
                id_route = self.get_routes().add_row(ids_trip=id_tripresult,
                                                     ids_edges=route,
                                                     #costs = duration_matched,
                                                     #probabilities = 1.0,
                                                     )
...
class Tripanalyses(am.ArrayObjman):
