...
# @file    mapmatching-08-save_failed.py
...
class Mapmatching(cm.BaseObjman):
    def __init__(self, ident, scenario=None,
                 name='Mapmatching', info='Mapmatching functionality.',
                 **kwargs):

        self._init_objman(ident=ident, parent=scenario,
                          name=name, info=info, **kwargs)

        attrsman = self.set_attrsman(cm.Attrsman(self))
        attrsman.do_not_save_attr(scenario.get_ident())
...
def load_mapmatching(filepath, parent=None, logger=None):
    # typically parent is the scenario
    mapmatching = cm.load_obj(filepath, parent=parent)
