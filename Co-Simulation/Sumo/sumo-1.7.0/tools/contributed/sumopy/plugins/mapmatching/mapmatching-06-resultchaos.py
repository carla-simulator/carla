...
# @file    mapmatching-06-resultchaos.py
...
class BirgilMatcher(Process):
    def __init__(self,  mapmatching, results=None, logger=None, **kwargs):
    ...
    def do(self):
        ...
        for id_trip, ids_point, id_mode in zip(ids_trip, trips.ids_points[ids_trip], ids_mode):
            ...
            logger.w("Matched %d/%d traces, %.2f%% comleted. Avg match time = %.2fs, Terminated in %.1fh" %
                     (n_trip_matched, n_trips, progress, time_match_trace_av, time_end_est/3600), key='message')
...
class PostMatchfilter(FilterMixin):
    ...
    def filter_ids(self):
        ...
        inds_eliminate = np.logical_or(
            trips.ids_route_matched[ids_selected] < 0,  # unsuccessful match
...
class GpsTrips(Trips):
    ...
    def delete_unselected(self):
        self.del_rows(self.select_ids(np.logical_not(self.are_selected.get_value())))
...
class Matchresults(cm.BaseObjman):
    def __init__(self, ident='simresults', mapmatching=None,
        ...
        self._init_objman(ident=ident, parent=mapmatching, name=name,
                          info=info, **kwargs)
        attrsman = self.set_attrsman(cm.Attrsman(self))
    ...
    def _init_attributes(self):
        ...
        self.tripresults = attrsman.add(cm.ObjConf(Tripresults(self, mapmatching.trips),
