    ...
    : Super(ObjectInitializer)
{
    ...
    CaServiceObj = std::make_unique<CaService>(RandomEngine);
...
void AV2XSensor::InitModel(AActor *NewOwner) {
    // Defensive: CaServiceObj is owned by a unique_ptr created in the
    // constructor and lives until destruction, so this is normally non-null.
    if (!CaServiceObj)
    {
        return;
    }
...
void AV2XSensor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    // Drop our entry from the shared static map so a destroyed sensor cannot
    // leave a dangling AActor* key for the surviving V2X sensors to dereference.
    AV2XSensor::mActorV2XDataMap.erase(this);

    // CaServiceObj / PathLossModelObj are std::unique_ptr: they are released
    // automatically in the destructor, which runs after UE's teardown clears
    // ownership (SetOwner(nullptr) -> InitModel). Do not reset them here, or
    // that teardown call would dereference a freed object.
