...
void ADVSCamera::PostPhysTick(UWorld *World, ELevelTick TickType, float DeltaTime)
{
  ...
  if (ROS2->IsEnabled())
  {
    ...
    auto Transform = (ParentActor) ? GetActorTransform().GetRelativeTransform(ParentActor->GetActorTransform()) : DataStream.GetSensorTransform();
    ROS2->ProcessDataFromDVS(DataStream.GetSensorType(), Transform, BufView, this);
  }
  #endif
  if (events.size() > 0)
  {
