...
void CaService::SetActor(UWorld *world, AActor *Owner)
{
    mWorld = world;
    mActorOwner = Owner;
    mCarlaEpisode = (world != nullptr) ? UCarlaStatics::GetCurrentEpisode(world) : nullptr;

    // The episode is unavailable during world teardown (e.g. when the sensor is
    // destroyed and UE clears its owner via SetOwner(nullptr)) or before the
    // simulation is ready. Defer initialization: CarlaActorInitialized() stays
    // false and PrePhysTick retries on a later tick.
    if (mCarlaEpisode == nullptr)
    {
        mCarlaActor = nullptr;
        mVehicle = nullptr;
        return;
    ...
    if (mCarlaActor != nullptr)
    {
        if (mCarlaActor->GetActorType() == FCarlaActor::ActorType::Vehicle)
        {
            ...
            UE_LOG(LogCarla, Verbose, TEXT("CaService:Initialize Vehicle type"));
                 ...
                 (mCarlaActor->GetActorType() == FCarlaActor::ActorType::Sensor))
        {
            ...
            UE_LOG(LogCarla, Verbose, TEXT("CaService:Initialize RSU type"));
        }

        mStationId = static_cast<long>(mCarlaActor->GetActorId());
    }
    mStationType = GetStationType();
    UE_LOG(LogCarla, Verbose, TEXT("CaService:Initialize station type %i"), mStationType);
}

void CaService::SetParams(const float GenCamMin, const float GenCamMax, const bool FixedRate)
{
    UE_LOG(LogCarla, Verbose, TEXT("CaService:SetParams function called"));
...
{
    UE_LOG(LogCarla, VeryVerbose, TEXT("V2XSensor: generating cam message"));
    ...
    const FVector &Accelerometer)const
{
    ...
    return carla::geom::Vector3D{
        (float) Accelerometer.X + mRandomEngine->GetNormalDistribution(Mean, StdDevAccel.X),
        (float) Accelerometer.Y + mRandomEngine->GetNormalDistribution(Mean, StdDevAccel.Y),
        (float) Accelerometer.Z + mRandomEngine->GetNormalDistribution(Mean, StdDevAccel.Z)};
}
...
float CaService::ComputeYawRate()const
{
    ...
    // Angular velocity of the owner in its local frame, in rad/s. Mirrors the
    // computation in AInertialMeasurementUnit (whose helper is not exposed).
    FVector AngularVelocity = FVector::ZeroVector;
    if (const auto *RootComponent = Cast<UPrimitiveComponent>(mActorOwner->GetRootComponent()))
    {
