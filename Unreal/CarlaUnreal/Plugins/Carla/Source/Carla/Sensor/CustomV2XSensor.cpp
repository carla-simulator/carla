...
void ACustomV2XSensor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    // Drop our queued messages from the shared static maps so a destroyed
    // sensor cannot leave a dangling AActor* key for the surviving sensors.
    {
        std::lock_guard<std::mutex> lock(v2xDataLock);
        gActorV2XDataMap.erase(GetSenderId());
        gActorV2XDataMapNextFrame.erase(GetSenderId());
    }

    // PathLossModelObj is a std::unique_ptr released automatically in the
    // destructor; not reset here so teardown paths never see a freed object.
