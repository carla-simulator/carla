 ...
 *
 * Renders an HD-map overlay (lane lines, road boundaries, road markings,
 * crosswalks, stop lines and dynamic actor bounding boxes) into a dedicated
 * scene capture, using ULineBatchComponent_CARLA for the batched primitives.
 * The lane-line and road-boundary polylines are reconstructed from
 * carla::road::Map (see Cosmos/CosmosRoadGeometry.h), shared with the Cosmos
 * static exporters.
 */
UCLASS()
...
protected:
  ...
  // Duplicate functions from DrawDebugHelpers to guarantee they work outside
  // the editor.
...
private:
  ...
  // Reconstruct the painted lane-line and road-boundary polylines from
  // road::Map and draw them as persistent lines (in Unreal centimeters).
  void DrawRoadGeometry(const UWorld* World, int DepthPriority, uint8 DistAlpha);
