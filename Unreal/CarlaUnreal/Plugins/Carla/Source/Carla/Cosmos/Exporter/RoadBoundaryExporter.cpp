...
bool URoadBoundaryExporter::ExportCosmosRoadBoundaries(UWorld* World, const FString& SessionId, const FString& OutFilePath, FString& OutError)
{
  ...
  // Boundary lane types to export (matching the ue4 Shoulder/Sidewalk/Median
  // selection). Combined as a bitmask for GenerateWaypointsInRoad.
  const int32_t BoundaryMask =
  ...
  const carla::road::Lane::LaneType BoundaryType =
      static_cast<carla::road::Lane::LaneType>(BoundaryMask);

  // Collect the set of road IDs. The const Map does not expose its road map
  // publicly, so derive the road set from per-road-entry boundary waypoints.
  std::set<carla::road::RoadId> RoadIds;
  for (const auto& Waypoint :
       CarlaMap->GenerateWaypointsOnRoadEntries(BoundaryType))
  {
  ...
  {
    // One waypoint per boundary lane at the lane entry of this road.
    const std::vector<carla::road::element::Waypoint> LaneEntries =
        CarlaMap->GenerateWaypointsInRoad(RoadId, BoundaryType);
    ...
    for (const carla::road::element::Waypoint& Entry : LaneEntries)
    {
      ...
      // the lane right vector to reconstruct the boundary outer-edge polyline.
      ...
      while (bMore)
      {
        ...
        if (HalfWidth > HalfLaneWidthEpsilon)
        {
          ...
          // Right lanes (positive direction) have their outer edge on the
