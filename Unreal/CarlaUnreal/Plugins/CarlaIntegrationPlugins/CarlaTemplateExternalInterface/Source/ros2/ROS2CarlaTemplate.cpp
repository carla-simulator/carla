...
// static fields
std::shared_ptr<ROS2CarlaTemplate> ROS2CarlaTemplate::_instance;

void ROS2CarlaTemplate::Enable(bool enable) {
  _enabled = enable;
  log_info("ROS2CarlaTemplate enabled: ", _enabled);
}

void ROS2CarlaTemplate::SetFrame(uint64_t frame) {
  _frame = frame;
}

void ROS2CarlaTemplate::SetTimestamp(double timestamp) {
  if (IsEnabled()){
    double integral;
...
void ROS2CarlaTemplate::Shutdown() {
...
void ROS2CarlaTemplate::RemoveActorRosPublishers(void *actor)
{
...
std::pair<std::shared_ptr<BasicPublisher>, std::shared_ptr<CarlaTransformPublisher>> ROS2CarlaTemplate::GetOrCreateSensor(carla::streaming::detail::stream_id_type id, void* actor) {
  auto it_publishers = _publishers.find(actor);
  auto it_transforms = _transforms.find(actor);
  std::shared_ptr<BasicPublisher> publisher {};
  ...
  if (it_publishers != _publishers.end()) {
    ...
    if (ros_name == "CarlaTemplateExternalInterface__") {
      ros_name.pop_back();
      ros_name.pop_back();
      ros_name += string_id;
...
void ROS2CarlaTemplate::ProcessDataFromTemplate(
    carla::streaming::detail::stream_id_type stream_id,
    const carla::geom::Transform sensor_transform,
    const char* data,
    void *actor) {

    log_info("Sensor CarlaTemplateExternalInterfaceSensor to ROS data: frame.", _frame, "stream.", stream_id);
    {
      auto sensors = GetOrCreateSensor(stream_id, actor);
      if (sensors.first) {
        sensors.first->SetData(data);
