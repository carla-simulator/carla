...
public:
	...
	/** Projection origin (latitude, longitude in degrees) of the next imports: positions in the
	 *  imported UStreetMap are centimetres east / south of it. Set it to the twin's datum
	 *  before importing so OSM nodes land in the baked level's frame. */
	static FVector2D LatLonOrigin;

	UFUNCTION(BlueprintCallable, Category = "StreetMap")
	static void SetLatLonOrigin(FVector2D Origin) { LatLonOrigin = Origin; }

	UFUNCTION(BlueprintCallable, Category = "StreetMap")
	static FVector2D GetLatLonOrigin() { return LatLonOrigin; }
protected:	
