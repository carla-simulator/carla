...
bool UStreetMapFactory::LoadFromOpenStreetMapXMLFile( UStreetMap* StreetMap, FString& OSMFilePath, const bool bIsFilePathActuallyTextBuffer, FFeedbackContext* FeedbackContext )
{
  ...
  UE_LOG(LogStreetMapImporting, Log, TEXT("LoadFromOpenStreetMapXMLFile ORIGIN SET  %s ."), *(LatLonOrigin.ToString()) );
	...
	UE_LOG(LogStreetMapImporting, Log, TEXT("OSM nodes %d (%d tagged): %d signs, %d trees, %d amenities; %d roads, %d buildings"),
		OSMFile.NodeMap.Num(), TaggedNodeCount, StreetMap->Signs.Num(), StreetMap->Trees.Num(), StreetMap->Amenities.Num(),
		StreetMap->Roads.Num(), StreetMap->Buildings.Num());
