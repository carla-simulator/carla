...
bool UUSDCARLAInterface::ImportUSD(
    ...
    bool bImportUnusedReferences, bool bImportAsBlueprint, bool bSimReadyVehicle)
{
  ...
  Settings.bSimReadyVehicle = bSimReadyVehicle;
  return FSimReadyUSDImporter::LoadUSD(Path, Dest, Settings);
}

TArray<FUSDCARLALight> UUSDCARLAInterface::GetUSDLights(const FString& Path)
{
  TArray<FUSDCARLALight> Result;
  pxr::UsdStageRefPtr Stage = FSimReadyUSDHelper::LoadUSDStageFromPath(Path);
  std::vector<pxr::UsdPrim> LightPrims;
  if (Stage)
  {
      const auto& PrimRange = Stage->Traverse();
      for (const auto& Prim : PrimRange) {
          if (Prim.IsA<pxr::UsdLuxLight>() && !Prim.IsA<pxr::UsdLuxRectLight>()) {
  ...
  for (pxr::UsdPrim& LightPrim : LightPrims)
  {
    ...
    pxr::GfVec3f PxColor;
    pxr::VtValue vtValue;
    pxr::UsdAttribute LightAttr = SimReadyUsdLuxLightCompat::GetLightAttr(Light.GetPrim(), Light.GetColorAttr(), true);
    if (LightAttr.Get(&vtValue)) {
    ...
    FVector Location = USDConvertPosition(Stage, USDConvert(PxLocation), true);
...
TArray<FUSDCARLAWheelData> UUSDCARLAInterface::GetUSDWheelData(const FString& Path)
{
  pxr::UsdStageRefPtr Stage = 
      FSimReadyUSDHelper::LoadUSDStageFromPath(Path);
      ...
      FUSDCARLAWheelData& OutWheelData) -> bool
  {
    ...
    if(!WheelPrim)
    {
      UE_LOG(LogSimReadyUsd, Warning, TEXT("Wheel prim fail"));
      return false;
    }
    ...
    {
      UE_LOG(LogSimReadyUsd, Warning, TEXT("Relationships fail: %d %d %d"), 
    ...
    {
      UE_LOG(LogSimReadyUsd, Warning, TEXT("Prims fail: %d %d %d"), 
    ...
    UE_LOG(LogSimReadyUsd, Warning, TEXT("USD values: %f %f %f %f %f %f %f %f %f"), 
