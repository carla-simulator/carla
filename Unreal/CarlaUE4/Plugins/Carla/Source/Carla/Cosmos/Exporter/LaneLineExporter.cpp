...
FString ULaneLineExporter::MakeStableLabelId(const FString& Uuid, const TArray<FVector>& Vertices)
{
  uint32 Crc = 0;
  if (Vertices.Num() > 0)
  {
...
TArray<ARoadSpline*> ULaneLineExporter::GetLaneLineSplines(UWorld* World)
{
  TArray<ARoadSpline*> LaneLineSplines;

  if (!World) return LaneLineSplines;
  ...
  for (TActorIterator<ARoadSpline> It(World); It; ++It)
  {
    ...
    // Filter for lane lines (Driving boundary type)
    // Based on CosmosControlSensor logic that uses LaneLines color for Driving types
    if (Spline->BoundaryType == ERoadSplineBoundaryType::Driving)
    {
      LaneLineSplines.Add(Spline);
    }
  }

  return LaneLineSplines;
}
    ...
    const FString& OptionalClipVerId)
{
  ...
  TSharedRef<FJsonObject> LabelData = MakeShared<FJsonObject>();
  {
    ...
    // polyline3d.vertices (lane lines use polyline3d)
    {
...
bool ULaneLineExporter::ExportCosmosLaneLines(UWorld* World, const FString& SessionId, const FString& OutFilePath, FString& OutError)
{
  ...
  FString LabelClassIdentifier = TEXT("lanelines:autolabels");
  FString LabelClassVersion    = TEXT("v0");
  FString CoordinateFrame      = TEXT("rig");
  FString LabelNameEnum        = TEXT("lane_line");
  ...
  // Get all lane line splines (Driving boundary type)
  TArray<ARoadSpline*> LaneLineSplines = GetLaneLineSplines(World);

  if (LaneLineSplines.Num() == 0)
  {
    OutError = TEXT("No lane line splines found in the world");
    return false;
  }
  ...
  TArray<TSharedPtr<FJsonValue>> LabelsArr;
  LabelsArr.Reserve(LaneLineSplines.Num());

  for (ARoadSpline* Spline : LaneLineSplines)
  {
    if (!Spline || !Spline->SplineComponent) continue;
    ...
    // Append this lane line as a label
    AppendLaneLineLabel(
  ...
  UE_LOG(LogCarla, Log, TEXT("Exported %d lane lines to %s"), LabelsArr.Num(), *OutFilePath);
  return true;
}