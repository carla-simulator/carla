...
void ULargeMapTileManager::ChangeWorldOrigin(FIntVector InSrcOrigin, FIntVector InDstOrigin)
{
  ...
  CarlaEpisode->SetCurrentMapOrigin(InDstOrigin);
...
void ULargeMapTileManager::OnLevelAddedToWorld(ULevel* InLevel, FIntVector& WorldOrigin)
{
  ...
  SpawnAssetsInTile(Tile, WorldOrigin);
}
...
void ULargeMapTileManager::SpawnAssetsInTile(FCarlaMapTile& Tile, FIntVector& WorldOrigin)
{
  if (Tile.TilesSpawned) return;

  FVector CurrentWorldOrigin(WorldOrigin);
