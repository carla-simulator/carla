    ...
    const FVector &Gyroscope)
{
  ...
  constexpr float Mean = 0.0f;
  Gyroscope += BiasGyro;
  return Gyroscope + BiasGyro + carla::geom::Vector3D {
      Gyroscope.X + RandomEngine->GetNormalDistribution(Mean, StdDevGyro.X),
      Gyroscope.Y + RandomEngine->GetNormalDistribution(Mean, StdDevGyro.Y),
      Gyroscope.Z + RandomEngine->GetNormalDistribution(Mean, StdDevGyro.Z)
  };
}
