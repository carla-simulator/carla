...
void AV2XSensor::InitModel(AActor *NewOwner) {
    UE_LOG(LogCarla, Warning, TEXT("V2XSensor::InitOwner() function called %p: CarlaActorInitialized %i"), NewOwner, CaServiceObj->CarlaActorInitialized());
...
{
    UE_LOG(LogCarla, Warning, TEXT("V2XSensor: called setowner with %p (old: %p)"), NewOwner, GetOwner());
