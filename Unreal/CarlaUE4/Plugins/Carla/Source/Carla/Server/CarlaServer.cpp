...
void FCarlaServer::FPimpl::BindActions()
{
  ...
  BIND_SYNC(set_manual_bones_mode) <<[this](
    ...
    bool bEnabled)->R<void>
      ...
      if (!ActorView.IsValid())
      {
        RESPOND_ERROR("unable to set set_manual_bones_mode: actor not found");
      }
      auto Pawn = Cast<APawn>(ActorView.GetActor());
      if (Pawn == nullptr)
      {
      ...
      Controller->SetManualBones(bEnabled);
      return R<void>::Success();
    };

  BIND_SYNC(send_bones_position) <<[this](
    cr::ActorId ActorId,
...
#define MAKE_RESULT(operation) return parse_result(c.actor, operation);
  ...
  auto command_visitor = carla::Functional::MakeRecursiveOverload(
    ...
    return result.GetError();
      ...
      [=](auto, const C::ApplyVehicleControl &c) {
    MAKE_RESULT(apply_control_to_vehicle(c.actor, c.control));
  },
      [=](auto, const C::ApplyWalkerControl &c) {
    MAKE_RESULT(apply_control_to_walker(c.actor, c.control));
      ...
      [=](auto, const C::ApplyAngularVelocity &c) {
    MAKE_RESULT(set_actor_angular_velocity(c.actor, c.angular_velocity));
  },
      [=](auto, const C::ApplyImpulse &c) {         MAKE_RESULT(add_actor_impulse(c.actor, c.impulse)); },
      [=](auto, const C::SetSimulatePhysics &c) {
