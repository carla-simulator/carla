...
Enables the use of offset for the conversion. The offset will move the origin position of the map. Default value is __False__. If enabled this setting overwrites part of the `proj_string` setting that define latitude and logitude origin. As result the OpenDRIVE will not contain the `proj_string`.  
...
This variable sets the coordinate origin in the center of the map. Default value is __False__. If enabled this setting overwrites part of the `proj_string` setting that define latitude and logitude origin. As result the OpenDRIVE will not contain the `proj_string`.  
...
Sets what Open Street Map roads will be exported to OpenDRIVE.  
    - **Parameters:**
        - `way_types` (_list(str)_) – The list of Open Street Map road types that should be exported to OpenDRIVE. Default value is `["motorway", "motorway_link", "trunk", "trunk_link", "primary", "primary_link", "secondary", "secondary_link", "tertiary", "tertiary_link", "unclassified", "residential"]`.  
...
Sets a list of Open Street Map roads that will not generate traffic lights even if `generate_traffic_lights` is enabled. This prevents link roads or high ways to generate traffic lights.  
    - **Parameters:**
        - `way_types` (_list(str)_) – The list of Open Street Map road types that will not generate traffic lights. Default value is `["motorway_link", "primary_link", "secondary_link", "tertiary_link"]`.  
