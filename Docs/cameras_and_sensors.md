...
| `motion_blur_intensity`       | float | 0.5    | Strength of motion blur. 1 is max and 0 is off |
| `motion_blur_max_distortion`  | float | 5.0    | Max distortion caused by motion blur, in percent of the screen width, 0 is off |
| `motion_blur_min_object_screen_size`  | float | 0.5    | Percentage of screen width objects must have for motion blur, lower value means less draw calls
