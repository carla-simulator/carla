...
| `r.PathTracing.DLSSRR.FeatureIdleReleaseSeconds` | `10.0` | Seconds a DLSS Ray Reconstruction / DLSS Super Resolution feature (one per distinct view/lens configuration) may sit unused before its GPU memory is released; `0` disables the sweep (features are then evicted only by the 8-entry LRU). | Each `rt_lens` spawn used to pin its DLSS-RR feature's GPU memory (measured ~342 MB) for the life of the server; a scene that spawned and destroyed many rt_lens sensors accumulated that cost. With the sweep, an idle feature (no capture for longer than this) is released and its memory reclaimed. A `sensor_tick` longer than this value re-creates the feature — and resets the denoiser's temporal history — on every capture; raise it (or set `enable_denoiser=False`) for sensors that tick slower than 10 s if that reset is undesirable. |
