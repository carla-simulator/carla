...
!!! Important
    This sensor is quite different than the rest. It is highly recommended to read the specific [rss documentation](adv_rss.md) before reading this.  
...
The RSS sensor calculates the RSS state of a vehicle and retrieves the current RSS Response as sensor data. The [carla.RssRestrictor](python_api.md#carla.RssRestrictor) will use this data to adapt a [carla.VehicleControl](python_api.md#carla.VehicleControl) before sending to a vehicle.  

These controllers can begenerated by an _Automated Driving_ stack or user input. For instance, hereunder there is a fragment of code from `PythonAPI/examples/manual_control_rss.py`, where the user input is modified using RSS when necessary.  

__1.__ Check if the __RssSensor__ generates a valid response containing restrictions.   
__2.__ Gather the current dynamics of the vehicle and the vehicle physics.  
__3.__ Apply restrictions to the vehicle control using the response from the RssSensor, the current dynamics and the vehicle physics.  
...
<td><a href="../python_api#roadboundariesmode">carla.RoadBoundariesMode</a></td>
...
<td><a href="../python_api#visualizationmode">carla.VisualizationMode</a></td>
<td>States the visualization of the RSS calculations. Default is <b>All</b>. <br> <b>Off</b><br><b>RouteOnly</b><br><b>VehicleStateOnly</b><br><b>VehicleStateAndRoute</b><br><b>All</b></td>
</table>
<br>
...
!!! Warning
    This sensor works fully on the client side. Changes on the attributes will have effect __after__ the _listen()_ has been called since no actual server side blueprint is available.
