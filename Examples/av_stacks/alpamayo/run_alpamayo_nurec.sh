...
NUREC_DIR="$HERE/../../../PythonAPI/examples/nvidia/nurec"
...
if [[ ! -x "$NUREC_PYTHON" ]]; then
    ...
    echo "Run PythonAPI/examples/nvidia/nurec/install_nurec.sh first." >&2
    exit 1
fi
if [[ ! -f "$NUREC_USDZ" ]]; then
    echo "NuRec scene not found at $NUREC_USDZ" >&2
    echo "Run PythonAPI/examples/nvidia/nurec/install_nurec.sh first or set NUREC_USDZ." >&2
    exit 1
fi
