...
* Optimized the ray-cast LiDARs (`sensor.lidar.ray_cast`, `sensor.lidar.ray_cast_semantic`, `sensor.lidar.hss_lidar`): a new `UCarlaLidarSubsystem` simulates them on a background task that overlaps with the other sensors' Game Thread work, multiple LiDARs run in parallel under a single physics read lock (`carla.Lidar.Batch 0` restores sequential simulation), and the LiDAR transform is now computed once per frame instead of per ray. LiDAR output is unchanged.
