...
* Ported the Project Chrono vehicle-dynamics integration from `ue4-dev`. `UChronoMovementComponent` and the `enable_chrono_physics` RPC were already present but unreachable: `Carla.Build.cs` threw `NotImplementedException` for the Chrono path and nothing built or linked the library. Chrono 6.0.0 is now built by a new `Chrono/` subproject (`ExternalProject_Add`, mirroring `Ros2Native/`), pinned to C++14 and compiled with the Unreal toolchain; its shared objects are staged into the Carla plugin's `Binaries/Linux` and linked by `Carla.Build.cs`, and `WITH_CHRONO` is propagated through `Options.def`. Enable it with `-DENABLE_CHRONO=ON`; `-DCARLA_CHRONO_SOURCE_PATH=<PATH>` replaces the old `--chrono-path` flag. Also restored `Vehicle.restore_physx_physics` (and the `UBaseCarlaMovementComponent::DisableSpecialPhysics` hook it needs), which was dropped from ue5-dev and left `enable_chrono_physics` with no way back to the default physics. Ported the `Co-Simulation/Chrono/Vehicles/sedan` JSON templates and `Docs/tuto_G_chrono.md`. ue4-dev vendored Chrono's entire `data/vehicle/sedan` directory (~15 MB of meshes, POV-Ray exports and Blender sources) for the sake of one tuned chassis centroid; here only the JSON is tracked and the build stages the meshes from the Chrono install. `PythonAPI/examples/manual_control_chrono.py` is the ue5-dev `manual_control.py` plus a CTRL+O Chrono toggle, rather than the stale ue4-dev fork. Moved the "Chrono physics does not support collisions yet" warning out of `DisableChronoPhysics` and back into the two collision handlers it belongs to (where ue4-dev has it), so a deliberate `restore_physx_physics` no longer reports a collision that did not happen.
