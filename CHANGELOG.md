...
* Added the UE5.8 ROS 2 map/RViz demo with latched OpenDRIVE data and safe autopilot traffic synchronization.
...
* The raster camera sensors (`sensor.camera.rgb`, `depth`, `semantic_segmentation`, `instance_segmentation`, `normals`) now recycle their GPU readback staging buffer through the per-sensor readback pool instead of allocating and freeing a fresh `FRHIGPUTextureReadback` (8 MB at 1080p, per camera) on every frame: `ImageUtil::ReadSensorImageDataAsyncFColor` never forwarded the pool, so the pool that `ReadSensorImageDataAsync` and the ray-traced / fisheye lens families already used was unreachable from the pinhole camera path. `ReadSensorImageDataAsyncFLinearColor` had the same gap and gets the same fix. The delivered images are unchanged; a GPU-bound rig's frame rate is unchanged too (5 x `sensor.camera.rgb` at 1080p, Town10HD_Opt, 20 vehicles: 11.0 fps / 91.0 ms delivered before, 11.0 fps / 90.6 ms after; 1 camera: 33.2 fps / 30.2 ms before, 33.4 fps / 29.9 ms after) — what it removes is the per-frame allocate/free traffic on the RHI thread.
* `av_stacks/autoware/run/run_carla_autoware.sh --with-rviz` now starts RViz on a CARLA-adapted copy of the stock config in classical mode too: the image panel shows `autoware_demo.py`'s front camera (`/sensing/camera/front/image`, override with `--rviz-image-topic`) instead of the disabled traffic-light debug overlay, and the initial view follows the ego (`ThirdPersonFollower` on `base_link`) instead of the stock top-down view on the static `viewer` frame.
* Added `PythonAPI/examples/rig_perf_demo.py`, a two-pass multi-camera rig benchmark: `measure` drives an autopilot vehicle with an N-camera rig (`sensor.camera.rgb` with optional DLSS Super Resolution, or `sensor.camera.rt_lens` with optional DLSS Ray Reconstruction) headless in synchronous mode and records the per-tick delivered-frame rate plus a recorder log of the drive; `render` replays that log with the same rig and writes a grid video whose per-frame FPS labels are the live pass-1 numbers.
* Fixed the DLSS Ray Reconstruction denoiser (`sensor.camera.rt_lens`, engine plugin `DLSSRRDenoiser`) resetting its temporal history on every capture of a live sensor slower than 10 s per frame: the NGX feature cache released features on a wall-clock idle timeout (`r.PathTracing.DLSSRR.FeatureIdleReleaseSeconds`, default 10), so a slow synchronous client or a sparse `sensor_tick` paid a feature re-create per capture. Feature lifetime is now tied to the sensor's view state (released within a second of the sensor being destroyed, never while it lives); the timeout is an opt-in fallback that defaults to 0.
* Fixed `world.set_sky_light_map` surviving `load_world()`/`reload_world()`: the environment-map override was process-wide and was re-applied to the freshly loaded level's sky rig on its first weather push, so a client that loaded a new map inherited the previous map's lighting (and `has_sky_light_map()` stayed true). The override is now scoped to the world it was set on and dropped when that world is cleaned up; set it again after loading a map.
