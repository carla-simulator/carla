...
* Fixed the Windows build: the setup now installs the MSVC 14.38 toolset required by the Unreal Engine 5.5.4 fork, MSVC builds use `/W4` instead of `/Wall` (whose warning flood broke googletest's `/WX` build), and two Windows compile/link errors in LibCarla were fixed (`independent_bits_engine` result type, `geom::Location` forward-declared as `class` instead of `struct`).
