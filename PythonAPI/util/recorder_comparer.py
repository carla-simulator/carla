...
def main():
    ...
    argparser.add_argument('-f1', '--file1', help='recorder filename (test1.txt)')
    argparser.add_argument('-f2', '--file2', help='recorder filename (test2.txt)')
    ...
    try:
        for i in range(len_1):
            ...
            # Ignore some lines
            if (row1.startswith('Parenting') and row2.startswith('Parenting')) or (
                row1.startswith('Collision') and row2.startswith('Collision')
            ):  # Based on ids, which vary
                continue
            if row1.startswith(('Destroy', 'Current platform time:')):  # No more info apart from the id, which vary
                continue

            # Ignoring the Id part, as it can differ between simulations
            if row1.startswith('Id:'):
            ...
            if row1_info != row2_info:
                ...
                print(
                    f' [Frame \033[1m{frame_1}{" " * max(0, FRAME_SPACES - len(frame_1))}\033[0m - Time \033[1m{time_1}{" " * max(0, TIME_SPACES - len(time_1))}\033[0m] -> (\033[1m'
                    + row1
                    + '\033[0m)'
                )
                print(
                    f' [Frame \033[1m{frame_2}{" " * max(0, FRAME_SPACES - len(frame_2))}\033[0m - Time \033[1m{time_2}{" " * max(0, TIME_SPACES - len(time_2))}\033[0m] -> (\033[1m'
                    + row2
                    + '\033[0m)'
                )
                input()
