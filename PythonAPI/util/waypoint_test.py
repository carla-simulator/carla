...
def draw_waypoint_list(debug, w_list, color, lt=5):
    for w in w_list:
        draw_transform(debug, w.transform, color, lt)
...
def main():
    ...
    try:
        ...
        next_until_end = list(current_w.next_until_lane_end(waypoint_separation))
        previous_until_start = list(current_w.previous_until_lane_start(waypoint_separation))

        draw_transform(debug, current_w.transform, blue, 10)
        draw_waypoint_list(debug, next_until_end, green, 10)
        draw_waypoint_list(debug, previous_until_start, red, 10)
