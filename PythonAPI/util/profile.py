...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        self.current_cull_distance = self.world.get_settings().max_culling_distance
    ...
    def update_culling_distance(self, reverse=False, short_distance=False):
        increment = 1.0 if not reverse else -1.0
        distance_offset = 1000.0 if not short_distance else 100.0
        settings = self.world.get_settings()
        settings.max_culling_distance += increment * distance_offset
        self.current_cull_distance = settings.max_culling_distance
        self.world.apply_settings(settings)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    world.update_culling_distance(True, pygame.key.get_mods() & KMOD_SHIFT)
                elif event.key == K_UP:
                    world.update_culling_distance(False, pygame.key.get_mods() & KMOD_SHIFT)
                elif event.key == K_F1:
                    world.toggle_hud()
                elif event.key == K_F2:
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        self._info_text = [
            ...
            'Current cull distance: %.2f' % world.current_cull_distance,
            'Car transform',
...
class CameraManager(object):
    def __init__(self, parent_actor, hud, num_cameras):
        self.sensor = []
        self._parent = parent_actor
        self.hud = hud
        self.recording = False
        self.num_sensors = num_cameras
        ...
        x = 0
        y = 0
        width_offset = int(self.hud.dim[0])
        height_offset = int(self.hud.dim[1])
        if self.num_sensors > 1:
            width_offset = int(width_offset/2)
...
class BenchmarkSensor(object):
    ...
    def calculate_results(self):
        ...
        print('DrawPrimitiveCalls: %d'% self.total["STATGROUP_RHICMDLIST"]["STAT_RHIDrawPrimitiveCalls"]["mean"])
        print("")
...
def main():
    ...
    argparser.add_argument(
        ...
        default=4,
        type=int,
        help='Num Camera Sensors (default: 4)')
    argparser.add_argument(
        '-a', '--autopilot',
        action='store_true',
