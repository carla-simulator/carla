...
def main():
    ...
    for bp_id in sorted(blueprint_ids):
        ...
        if bp_type in bp_dict:
            bp_dict[bp_type].append(temp)
        else:
            bp_dict[bp_type] = [temp]

    md = MarkdownFile()
    md.not_title('Blueprint Library')
    md.textn(
        "The Blueprint Library ([`carla.BlueprintLibrary`](../python_api/#carlablueprintlibrary-class))" +
        "is a summary of all [`carla.ActorBlueprint`](../python_api/#carla.ActorBlueprint) and its attributes " +
