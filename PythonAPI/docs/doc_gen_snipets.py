    ...
    def gen_body(self):
        ...
        for snipet_path in self._snipets:
            ...
            md.textn(
                ...
                'onclick="CopyToClipboard(\'' + snipet_name + '-code\')">Copy snippet</button>'
                '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
                '<button id="button1" class="CloseSnipet" '
                'onclick="CloseSnipet()">Close snippet</button><br><br>\n'
            )
...
def main():
    ...
    snipets_images_path = (
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) + '/Docs/img/snipets_images'
    )
    docs = Documentation(script_path, snipets_images_path)
    snipets_md_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(script_path))), 'Docs/python_api_snipets.md'
    )
