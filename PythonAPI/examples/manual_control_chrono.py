...
"""Allows controlling a vehicle with a keyboard, with Project Chrono vehicle dynamics.

Requires a server built with -DENABLE_CHRONO=ON. See Docs/tuto_G_chrono.md.
"""
            ...
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    if not self._chrono_enabled:
                        # Paths are relative to base_path; the Chrono templates
                        # cross-reference each other from that same root.
                        ...
                        base_path = str(
                            Path(__file__).resolve().parents[2] / "Co-Simulation" / "Chrono" / "Vehicles") + os.sep
                        world.player.enable_chrono_physics(
                            5000, 0.002, vehicle_json, powertrain_json, tire_json, base_path)
