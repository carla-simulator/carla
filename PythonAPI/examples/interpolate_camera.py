    ...
    for real_tree in base_tree.iter('interpolations'):
        for interpolation in real_tree.iter('interpolation'):
            ...
            for point in interpolation.iter('point'):
                ...
                interpolation_key_points.append(
                    [time, carla.Location(x=x, y=y, z=z), carla.Rotation(pitch=pitch, yaw=yaw, roll=roll)]
                )
...
def main():
    ...
    argparser.add_argument('-f', '--file', required=True, help='Interpolation file')
    argparser.add_argument('--id', required=True, help='Id to be run part of the interpolation file.')
    argparser.add_argument('--debug', action='store_true', help='Debug interpolation data.')
    argparser.add_argument('-st', '--start-time', default=0, type=int, help='Starting keypoint of the interpolation.')
    argparser.add_argument('--sync', action='store_true', help='Activates synchronous mode.')
    ...
    try:
        ...
        if args.debug:
            ...
            for point in key_points:
                ...
                text = (
                    f'Keypoint: {point[0]}, '
                    f'{round(point[1].x, 2)!s}, '
                    f'{round(point[1].y, 2)!s}, '
                    f'{round(point[1].z, 2)!s}, '
                    f'{round(point[2].roll, 2)!s}, '
                    f'{round(point[2].yaw, 2)!s}, '
                    f'{round(point[2].pitch, 2)!s}'
                )
                world.debug.draw_string(
                    point[1] + carla.Location(z=1), text, color=carla.Color(0, 128, 0), life_time=LIFETIME
                )
            ...
            for i, transform in enumerate(data):
                world.debug.draw_point(
                    transform.location, size=0.05, life_time=LIFETIME, color=carla.Color(128, 0, 128)
                )
                forward_vector = transform.get_forward_vector()
                world.debug.draw_arrow(
                    transform.location,
                    transform.location + 2 * forward_vector,
                    thickness=0.03,
                    arrow_size=0.03,
                    life_time=LIFETIME,
