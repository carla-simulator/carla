...
param transition_length = 38.
behavior EgoBehavior():
    state = {}
    do SmoothKeep(globalParameters.wait_speed * KMH, state, leads=[barrier]) until fwd_lat(self, passer)[0] > 28
    target = self.laneSection._laneToLeft.lane
    do SmoothKeep(globalParameters.change_speed * KMH, state, lane=target, transition=globalParameters.transition_length, leads=[barrier, passer]) until lateral_to_lane(self, target) < .25
    do SmoothKeep(globalParameters.ego_speed * KMH, state, lane=target, leads=[passer])
