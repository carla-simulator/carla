...
def argument_parser():
    ...
    argparser.add_argument(
        '--safe', type=bool, default=True, help='Avoid spawning vehicles prone to accidents (default True)'
    )
    argparser.add_argument(
    ...
    argparser.add_argument('-s', '--seed', metavar='S', type=int, help='Set random seed')
    argparser.add_argument('--hero', action='store_true', default=False, help='Set one of the vehicles as hero')
    argparser.add_argument('--iai-key', type=str, help='InvertedAI API key.')
    argparser.add_argument(
        '--record', action='store_true', help='Record the simulation using the CARLA recorder', default=False
    )
    argparser.add_argument(
        '--sim-length', type=int, default=120, help='Length of the simulation in seconds (default: 120)'
    )
    argparser.add_argument(
        '--location',
        type=str,
        help='IAI formatted map on which to create simulate (default: carla:Town10HD)',
        default='carla:Town10HD',
    )
    argparser.add_argument(
        '--capacity',
        type=int,
        help='The capacity parameter of a quadtree leaf before splitting (default: 100)',
        default=100,
    )
    argparser.add_argument('--width', type=int, help='Full width of the area to initialize (default: 250)', default=250)
    argparser.add_argument(
        '--height', type=int, help='Full height of the area to initialize (default: 250)', default=250
    )
    argparser.add_argument(
        '--map-center',
        type=int,
        nargs='+',
        help='Center of the area to initialize (default: [0,30])',
        default=(0, 30),
    )
    argparser.add_argument(
        '--iai-async', type=bool, help='Whether to call drive asynchronously (default: True)', default=True
    )
    argparser.add_argument('--api-model', type=str, help='IAI API model version (default: bI5p)', default='bI5p')
    argparser.add_argument(
        '--iai-log',
        action='store_true',
...
def set_spectator(world, hero_v):
    spectator_offset_x = -6.0
    spectator_offset_z = 6.0
...
def initialize_iai_agent(actor, agent_type):
    ...
    speed = math.sqrt(vel.x**2.0 + vel.y**2.0 + vel.z**2.0)

    agent_state = AgentState.fromlist([transf.location.x, transf.location.y, transf.rotation.yaw, speed])
...
def transform_iai_to_carla(agent_state):
    return carla.Transform(
        carla.Location(agent_state.center.x, agent_state.center.y, 0.0),
        carla.Rotation(yaw=math.degrees(agent_state.orientation)),
    )
...
def assign_carla_blueprints_to_iai_agents(
    world, vehicle_blueprints, agent_properties, agent_states, recurrent_states, is_iai, noniai_actors
):
    ...
    for agent_id, state in enumerate(agent_states):
        if not is_iai[agent_id]:
            ...
            if actor is not None:
                ...
                iai2carla[len(iai2carla)] = {
                    'actor': actor,
                    'is_iai': True,
                    'type': agent_properties[agent_id].agent_type,
                }
...
def initialize_simulation(args, world, agent_states=None, agent_properties=None):
    ...
    location_info_response = iai.location_info(location=args.location, rendering_center=map_center)
    ...
    # Place vehicles within the specified regions which will consider the
    # relative states of nearby vehicles in neighbouring regions.
...
def main():
    ...
    try:
        ...
        print(
            '\n\tYou need to indicate the InvertedAI API key with the argument '
            '--iai-key. To obtain one, please go to https://www.inverted.ai \n'
        )

    num_pedestrians = args.number_of_walkers

    FPS = int(1.0 / world.get_settings().fixed_delta_seconds)
    ...
    # Initialize InvertedAI co-simulation
    response, carla2iai_tl, location_info_response = initialize_simulation(
        args, world, agent_states=agent_states, agent_properties=agent_properties
    )
    ...
    if args.iai_log:
        ...
        log_writer.initialize(
            location=args.location, location_info_response=location_info_response, init_response=response
        )
        iailog_path = os.path.join(os.getcwd(), 'iai_log.json')

    # Map IAI agents to CARLA actors and update response properties and states
    agent_properties, agent_states_new, recurrent_states_new, iai2carla = assign_carla_blueprints_to_iai_agents(
        world,
        vehicle_blueprints,
        agent_properties,
    ...
    try:
        ...
        for _frame in range(args.sim_length * FPS):
            response.traffic_lights_states = assign_iai_traffic_lights_from_carla(
                world, response.traffic_lights_states, carla2iai_tl
            )
            ...
            # Include possible new actors (vehicles) from other clients
            # (using automatic_control.py or manual_control.py for instance)
            actors_all = world.get_actors().filter('vehicle.*')
            actsids = [act['actor'].id for act in iai2carla.values()]
            for actor in actors_all:
                if actor.id not in actsids:
