...
class TwoCarsHighSpeedCollision(Scenario):
    def init_scene(self, prefix, settings=None, spectator_tr=None):
        ...
        batch = [
            SpawnActor(vehicle00_bp, vehicle00_tr).then(ApplyTargetVelocity(FutureActor, carla.Vector3D(-50, 0, 0))),
            SpawnActor(vehicle01_bp, vehicle01_tr).then(ApplyTargetVelocity(FutureActor, carla.Vector3D(+50, 0, 0))),
        ]
...
class ThreeCarsSlowSpeedCollision(Scenario):
    def init_scene(self, prefix, settings=None, spectator_tr=None):
        ...
        batch = [
            SpawnActor(vehicle00_bp, vehicle00_tr).then(ApplyTargetVelocity(FutureActor, carla.Vector3D(-15, 0, 0))),
            SpawnActor(vehicle01_bp, vehicle01_tr).then(ApplyTargetVelocity(FutureActor, carla.Vector3D(+15, 0, 0))),
            SpawnActor(vehicle02_bp, vehicle02_tr).then(ApplyTargetVelocity(FutureActor, carla.Vector3D(0, -15, 0))),
        ]
...
class CarBikeCollision(Scenario):
    def init_scene(self, prefix, settings=None, spectator_tr=None):
        ...
        batch = [
            SpawnActor(car_bp, car_tr).then(ApplyTargetVelocity(FutureActor, carla.Vector3D(30, 0, 0))),
            SpawnActor(bike_bp, bike_tr).then(ApplyTargetVelocity(FutureActor, carla.Vector3D(0, -12, 0))),
        ]
...
class CarWalkerCollision(Scenario):
    def init_scene(self, prefix, settings=None, spectator_tr=None):
        ...
        batch = [
            SpawnActor(car_bp, car_tr).then(ApplyTargetVelocity(FutureActor, carla.Vector3D(20, 0, 0))),
    ...
    def test_scenario(self, fps=20, fps_phys=100, repetitions=1, sim_tics=100):
        ...
        if determ_repet[0] != repetitions:
            raise DeterminismError(
                'CollisionTransfError: Scenario %s is not deterministic: %d / %d'
...
class TestCollisionDeterminism(SmokeTest):
    ...
    def test_two_cars(self):
        ...
        try:
            test_collision = CollisionScenarioTester(
                scene=TwoCarsHighSpeedCollision(self.client, self.world, True), output_path=output_path
            )
    ...
    def test_three_cars(self):
        ...
        try:
            test_collision = CollisionScenarioTester(
                scene=ThreeCarsSlowSpeedCollision(self.client, self.world, True), output_path=output_path
            )
            test_collision.test_scenario(repetitions=5, sim_tics=100)
        except DeterminismError as err:
            test_collision.scene.clear_scene()
    ...
    def test_car_bike(self):
        ...
        try:
            test_collision = CollisionScenarioTester(
                scene=CarBikeCollision(self.client, self.world, True), output_path=output_path
            )
            test_collision.test_scenario(repetitions=5, sim_tics=100)
        except DeterminismError as err:
            test_collision.scene.clear_scene()
    ...
    def test_car_walker(self):
        ...
        try:
            test_collision = CollisionScenarioTester(
                scene=CarWalkerCollision(self.client, self.world, True), output_path=output_path
            )
            test_collision.test_scenario(repetitions=5, sim_tics=100)
        except DeterminismError as err:
            test_collision.scene.clear_scene()
