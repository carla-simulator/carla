...
def main():
    ...
    parser.add_argument(
        '--res', metavar='WIDTHxHEIGHT', default='1920x1080', help='window resolution (default: 1920x1080)'
    )
    parser.add_argument('--filter', metavar='PATTERN', default='lincoln', help='actor filter (default: "lincoln")')
    parser.add_argument('--sync', action='store_true', help='Activate synchronous mode execution')
    parser.add_argument('--spawn-point', type=int, default=0, help='Vehicle spawn point index.')
    parser.add_argument('--gamma', default=1.0, type=float, help='Gamma correction of the camera (default: 1.0)')
    parser.add_argument('--sensor', type=str, default='sensor.camera.rgb', help='Sensor ID')
    parser.add_argument(
        '-a',
        '--agent',
        type=str,
    ...
    parser.add_argument('-s', '--seed', type=int, default=0xCDCDCDCD, help='PRNG seed value')
    parser.add_argument('-n', '--frame-count', type=int, default=100, help='Number of frames to save')
    parser.add_argument('-w', '--warmup-frame-count', type=int, default=10, help='Number of frames to save')
    parser.add_argument('-g', '--generate', action='store_true', help='Whether to generate the ground truth file.')
    ...
    try:
        ...
        frame_paths = [file for file in OUTPUT_DIR.iterdir() if file.name.endswith('.png')]
        ...
        if argv.generate:
            ...
            video = cv2.VideoWriter(OUTPUT_FILE, fourcc, 60, (width, height))
            ...
            if argv.plot:
                ...
                plt.plot(np.linspace(0, len(correlation_results), len(correlation_results)), correlation_results)
                plt.plot(np.linspace(0, len(bhattacharyya_results), len(bhattacharyya_results)), bhattacharyya_results)
                plt.show()
            print(
                f'Final results:'
