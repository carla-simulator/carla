...
argparser.add_argument(
    '--host', metavar='H', default=DEFAULT_HOST, help=f'IP of the host server (default: {DEFAULT_HOST})'
)
argparser.add_argument(
    '-p', '--port', metavar='P', default=DEFAULT_PORT, type=int, help=f'TCP port to listen to (default: {DEFAULT_PORT})'
)
...
class TestApplyTextures(unittest.TestCase):
    ...
    def test_apply_textures(self, host=DEFAULT_HOST, port=DEFAULT_PORT):
        try:
            ...
            noise_data = np.reshape([np.abs(np.random.normal()) for i in range(HEIGHT * WIDTH)], [WIDTH, HEIGHT])
            ...
            camera = world.spawn_actor(bpl.find('sensor.camera.rgb'), transform)
            ...
            for e in world.get_actors():
                ...
                e.apply_texture(carla.MaterialParameter.Diffuse, noise_texture_color_red)
            time.sleep(2)
        finally:
