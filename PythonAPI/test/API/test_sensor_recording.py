...
class Scenario:
    def __init__(self, test):
        ...
        spectator.set_transform(
            carla.Transform(carla.Location(39.32, 130.43, 5.84), carla.Rotation(pitch=-29, yaw=180))
        )
        player_bp_to_spawn = bp_lib.find('static.prop.vendingmachine')
        self.player = self.world.try_spawn_actor(
            player_bp_to_spawn, carla.Transform(carla.Location(32.338, 130.16, 0.6), carla.Rotation(yaw=180))
        )
...
class CameraManager:
    def __init__(self, parent_actor, unit_test, start_time):
        ...
        self.sensors = [
            ...
            [
                SensorType.Depth_LogScale,
                'sensor.camera.depth',
                cc.LogarithmicDepth,
                'CameraDepth (LogarithmicGrayScale)',
                {},
            ],
            [
                SensorType.SemanticSeg_Raw,
                'sensor.camera.semantic_segmentation',
                cc.Raw,
                'CameraSemanticSegmentation (Raw)',
                {},
            ],
            [
                SensorType.SemanticSeg_CityScapesPalette,
                'sensor.camera.semantic_segmentation',
                cc.CityScapesPalette,
                'CameraSemanticSegmentation (CityScapesPalette)',
                {},
            ],
            [
                SensorType.InstanceSeg_Raw,
                'sensor.camera.instance_segmentation',
                cc.Raw,
                'CameraInstanceSegmentation (Raw)',
                {},
            ],
            [
                SensorType.InstanceSeg_CityScapesPalette,
                'sensor.camera.instance_segmentation',
                cc.CityScapesPalette,
                'CameraInstanceSegmentation (CityScapesPalette)',
                {},
            ],
            ...
            [
                ...
                None,
                'SemanticLidar (Ray-Cast)',
                {'range': '50'},
            ],
            [
                SensorType.RGB_Distorted,
                'sensor.camera.rgb',
                cc.Raw,
    ...
    def spawn_sensor(self, sensor_type):
        for sensor_item in self.sensors:
            if sensor_type == sensor_item[0]:
                ...
                spawned_sensor.listen(
                    lambda image, sensor_name=sensor_name, sensor_color=sensor_color: CameraManager._parse_image(
                        weak_self, image, sensor_name, sensor_color
                    )
                )
                break
    ...
    def _parse_image(weak_self, image, sensor_name, sensor_color):
        ...
        if sensor_color in (cc.Raw, None):
