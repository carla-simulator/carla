...
class TestVehicleControl(unittest.TestCase):
    ...
    def test_named_args(self):
        c = carla.VehicleControl(throttle=1.0, steer=2.0, brake=3.0, hand_brake=True, reverse=True)
        self.assertEqual(c.throttle, 1.0)
        self.assertEqual(c.steer, 2.0)
        self.assertEqual(c.brake, 3.0)
        self.assertEqual(c.hand_brake, True)
...
class TestVehiclePhysicsControl(unittest.TestCase):
    def test_named_args(self):
        torque_curve = [[0, 400], [24, 56], [24, 56], [1315.47, 654.445], [5729, 400]]
        ...
        wheels = [
            carla.WheelPhysicsControl(tire_friction=2, max_steer_angle=30, radius=10),
        ...
        error = 0.001
        for i in range(len(torque_curve)):
        ...
        for i in range(len(steering_curve)):
