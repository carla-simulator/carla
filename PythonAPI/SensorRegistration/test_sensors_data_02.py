...
class World(object):
    ...
    def restart(self):
        ...
        self.obstacle_detection = ObstacleDetection(self.player, self.hud)
    ...
    def destroy(self):
        ...
        sensors = [
            ...
            self.obstacle_detection.sensor]
...
class ObstacleDetection(object):
    def __init__(self, parent_actor, hud):
        ...
        if parent_actor.type_id.startswith("vehicle."):
            ...
            bp.set_attribute('hit_radius','0.75')
            ...
            # reference.
            weak_self = weakref.ref(self)
            self.sensor.listen(lambda event: ObstacleDetection._on_obstacle_detection(weak_self, event, world))
    
    @staticmethod
    def _on_obstacle_detection(weak_self, event, world):
        self = weak_self()
        if not self:
            return
        timestamp = world.get_snapshot()

        tolerance = 0.8*self._parent.bounding_box.extent.x # distance from boundingbox center to front of vehicle
        ...
        if 'static' not in event.other_actor.type_id:
            log_obstacle.append([
                ...
                event.distance-tolerance
            ])
            distance = str(event.distance-tolerance)
            self.hud.notification("Object ahead in %s" % distance)
            print(log_obstacle)
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            print(intersection_otherlane)
            print(intersection_offroad)'''

            timestamp = sim_world.get_snapshot()
            ...
            log.append([
                ...
                math.sqrt(acceleration.x**2 + acceleration.y**2 + acceleration.z**2), # note that acceleration.z==9.8 when standing due to gravity
        ...
        df = pd.DataFrame(log, columns=[
            ...
            "weather_sun_azimuth_angle",
            "weather_sun_altitude_angle",
            "weather_fog_density",
            "weather_fog_distance",
            "weather_fog_falloff",
            "weather_wetness",
            "weather_scattering_intensity",
            "weather_mie_scattering_scale",
            "weather_rayleigh_scattering_scale",
            # player information 
            "player_id", 
            "player_type_id", 
            "player_location_x", 
            ...
            "player_hand_brake", 
            "player_reverse", 
            "player_manual_gear_shift", 
            "player_gear"])
        # filename = "sensor_data\logs" + time.strftime("%Y-%m-%d_%H-%M-%S") + ".xlsx"
        # df.to_excel(filename)
        #save obstacle logs
        df_obstacles = pd.DataFrame(log_obstacle, columns=[
            # time information
            "frame_count", 
            "elapsed_seconds", 
            "delta_seconds", 
        ...
        df = df.merge(df_obstacles, on="frame_count", how="left")
        filename = '/home/kilianhullen/repos/carla/PythonAPI/SensorRegistration/test_data/' + time.strftime("%Y-%m-%d_%H-%M-%S") + ".log"
        df.to_excel(filename)

        if original_settings:
            print("apply original settings")
