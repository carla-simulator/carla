...
for PY_VERSION in ${PY_VERSION_LIST[@]} ; do
  ...
  if [[ -d "${ADRSS_INSTALL_DIR}" && -d "${ADRSS_BUILD_DIR}" ]]; then
    ...
    if [[ "${CMAKE_PREFIX_PATH}" == "" ]]; then
      CMAKE_PREFIX_PATH="${CARLA_BUILD_FOLDER}/boost-1.90.0-$CXX_TAG-install;${CARLA_BUILD_FOLDER}/proj-install"
    else
      CMAKE_PREFIX_PATH="${CARLA_BUILD_FOLDER}/boost-1.90.0-$CXX_TAG-install;${CARLA_BUILD_FOLDER}/proj-install;${CMAKE_PREFIX_PATH}"
    fi
