...
if $DO_PACKAGE ; then
  ...
  ${UE4_ROOT}/Engine/Build/BatchFiles/RunUAT.sh BuildCookRun \
      ...
      -targetplatform=Mac -build -CrashReporter -utf8output \
...
if [[ ! -d ${BUILD_FOLDER}/MacNoEditor ]] ; then
...
if $DO_COPY_FILES ; then
  ...
  DESTINATION=${BUILD_FOLDER}/MacNoEditor
...
if $DO_TARBALL ; then
  ...
  SOURCE=${BUILD_FOLDER}/MacNoEditor

  pushd "${SOURCE}" >/dev/null

  log "Packaging build."

  rm -f ./Manifest_NonUFSFiles_Mac.txt
