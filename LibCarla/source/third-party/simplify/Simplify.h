      ...
      for (int iteration = 0; iteration < 100; iteration++)
      {
        ...
        loopi(0, triangles.size())
        {
          ...
          loopj(0, 3) if (t.err[j] < threshold)
          {
            ...
            deleted0.resize(static_cast<size_t>(v0.tcount));
            deleted1.resize(static_cast<size_t>(v1.tcount));
    ...
    void update_mesh(int iteration)
    {
      ...
      if (iteration == 0)
      {
        ...
        loopi(0, vertices.size())
        {
          ...
          {
            int tid_idx = refs[static_cast<size_t>(v.tstart) + j].tid;
            Triangle &t = triangles[static_cast<size_t>(tid_idx)];
            loopk(0, 3)
            {
              size_t ofs = 0;
              int id = t.v[k];
