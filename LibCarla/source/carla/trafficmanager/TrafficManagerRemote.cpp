...
void TrafficManagerRemote::RegisterVehicles(
    const std::vector<ActorPtr> &_actor_list) {
...
void TrafficManagerRemote::UnregisterVehicles(
    const std::vector<ActorPtr> &_actor_list) {
  /// Prepare rpc actor list
  std::vector<carla::rpc::Actor> actor_list;
  for (auto &&actor : _actor_list) {
...
void TrafficManagerRemote::SetPercentageSpeedDifference(
    const ActorPtr &_actor, const float percentage) {
  /// Prepare rpc actor list
...
void TrafficManagerRemote::SetGlobalPercentageSpeedDifference(
...
void TrafficManagerRemote::SetForceLaneChange(
...
void TrafficManagerRemote::SetAutoLaneChange(
    const ActorPtr &_actor,
    const bool enable) {
  /// Prepare rpc actor list
  carla::rpc::Actor actor(_actor->Serialize());
...
/// the leading vehicle.
void TrafficManagerRemote::SetDistanceToLeadingVehicle(
    const ActorPtr &_actor,
...
void TrafficManagerRemote::SetPercentageIgnoreVehicles(
    const ActorPtr &_actor,
    const float percentage) {
  /// Prepare rpc actor list
  carla::rpc::Actor actor(_actor->Serialize());
...
void TrafficManagerRemote::SetPercentageRunningLight(
    const ActorPtr &_actor,
    const float percentage) {
  /// Prepare rpc actor list
  carla::rpc::Actor actor(_actor->Serialize());
