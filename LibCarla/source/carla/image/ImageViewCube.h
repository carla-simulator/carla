...
namespace image {
  ...
  private:
    ...
    static auto MakeViewCubeFromSensorImage(ImageT &image) {
      ...
      return interleaved_view(
          ...
          sizeof(sd::Color) * image.GetWidth());
    }

  public:

    template <typename ImageT>
    static auto MakeViewCube(ImageT &image) {
      return boost::gil::view(image);
    }

    static auto  MakeViewCube(sensor::data::ImageTmplCube<sensor::data::Color> &image) {
      return MakeViewCubeFromSensorImage<boost::gil::bgra8_pixel_t>(image);
    }

    static auto  MakeViewCube(const sensor::data::ImageTmplCube<sensor::data::Color> &image) {
      return MakeViewCubeFromSensorImage<boost::gil::bgra8c_pixel_t>(image);
    }
