  ...
  void GnssSensor::Listen(CallbackFunctionType callback) {
    ...
    for (auto element: geo_ref_splitted) {
      ...
      if (geo_ref_key_value.size() != 2u) {
        throw std::runtime_error(GetDisplayId() + ": invalid geo reference: '" + map->GetGeoReference() + "'");
      }
      ...
      if (istr.fail() || !istr.eof()) {
        throw std::runtime_error(GetDisplayId() + ": cannot parse geo reference: '" + map->GetGeoReference() + "'");
      }
    }
    ...
    if (std::isnan(_map_latitude) || std::isnan(_map_longitude)) {
      throw std::runtime_error(GetDisplayId() + ": cannot parse georeference: '" + map->GetGeoReference() + "'");
    }
