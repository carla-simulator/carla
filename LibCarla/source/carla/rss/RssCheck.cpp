...
void printRoute(std::string const &route_descr,
  ...
  for (auto road_segment : route.roadSegments) {
    for (auto lane_segment : road_segment.drivableLaneSegments) {
      std::cout << "(" << static_cast<uint64_t>(lane_segment.laneInterval.laneId)
                << " | " << std::setprecision(2)
...
RssCheck::RssCheck()
  : _ego_vehicle_dynamics(GetDefaultVehicleDynamics()),
    ...
    _road_boundaries_mode(RoadBoundariesMode::Off),
    _visualization_mode(VisualizationMode::All) {
  _logger = spdlog::get("RssCheck");
  if (!_logger) {
    _logger = spdlog::stdout_color_mt("RssCheck");
  }
  ...
  _timing_logger = spdlog::get("RssCheckTiming");
  if (!_timing_logger) {
    _timing_logger = spdlog::stdout_color_mt("RssCheckTiming");
  }
...
::ad::rss::world::RssDynamics RssCheck::GetDefaultVehicleDynamics() const {
...
void RssCheck::SetEgoVehicleDynamics(
...
void RssCheck::SetOtherVehicleDynamics(
    const ::ad::rss::world::RssDynamics &other_vehicle_dynamics) {
  _other_vehicle_dynamics = other_vehicle_dynamics;
}

const ::carla::rss::RoadBoundariesMode &RssCheck::GetRoadBoundariesMode()
    const {
  return _road_boundaries_mode;
}

void RssCheck::SetRoadBoundariesMode(
    const ::carla::rss::RoadBoundariesMode &road_boundaries_mode) {
  _road_boundaries_mode = road_boundaries_mode;
}

void RssCheck::AppendRoutingTarget(
    ::carla::geom::Transform const &routing_target) {
  _routing_targets_to_append.push_back(::ad::map::point::createENUPoint(
      routing_target.location.x, -1. * routing_target.location.y, 0.));
}

const std::vector<::carla::geom::Transform> RssCheck::GetRoutingTargets()
    const {
...
void RssCheck::SetVisualizationMode(
  ...
  try {
    double const time_since_epoch_check_start_ms =
        std::chrono::duration<double, std::milli>(
            ...
            .count();
#if DEBUG_TIMING
    std::cout << "--- time: " << timestamp.frame << ", "
              << timestamp.elapsed_seconds << std::endl;
    auto t_start = std::chrono::high_resolution_clock::now();
    auto t_end = std::chrono::high_resolution_clock::now();
    std::cout
        << "-> SC "
        << std::chrono::duration<double, std::milli>(t_end - t_start).count()
        << " start checkObjects" << std::endl;
#endif

    const auto carla_ego_vehicle =
        boost::dynamic_pointer_cast<carla::client::Vehicle>(carla_ego_actor);
    ...
    auto const ego_match_object =
        GetMatchObject(*carla_ego_vehicle, ::ad::physics::Distance(2.0));

    if (::ad::map::point::isValid(
            _carla_rss_state.ego_match_object.enuPosition.centerPoint, false)) {
    ...
    _carla_rss_state.ego_dynamics_on_route = CalculateEgoDynamicsOnRoute(
        timestamp, time_since_epoch_check_start_ms, *carla_ego_vehicle,
        _carla_rss_state.ego_match_object, _carla_rss_state.ego_route,
...
::ad::map::match::Object RssCheck::GetMatchObject(
  ...
  match_object.enuPosition.centerPoint.x =
      ::ad::map::point::ENUCoordinate(vehicle_transform.location.x);
  match_object.enuPosition.centerPoint.y =
      ::ad::map::point::ENUCoordinate(-1 * vehicle_transform.location.y);
  match_object.enuPosition.centerPoint.z =
      ::ad::map::point::ENUCoordinate(0);  // vehicle_transform.location.z;
  match_object.enuPosition.heading = ::ad::map::point::createENUHeading(
      (-1 * vehicle_transform.rotation.yaw) * to_radians);

  const auto &bounding_box = carla_vehicle.GetBoundingBox();
  match_object.enuPosition.dimension.length =
      ::ad::physics::Distance(2 * bounding_box.extent.x);
  match_object.enuPosition.dimension.width =
      ::ad::physics::Distance(2 * bounding_box.extent.y);
  match_object.enuPosition.dimension.height =
      ::ad::physics::Distance(2 * bounding_box.extent.z);
...
::ad::physics::Speed RssCheck::GetSpeed(
...
void RssCheck::UpdateRoute(CarlaRssState &carla_rss_state) {
  ...
  for (auto reference_point :
    ...
    auto const &reference_position =
        carla_rss_state.ego_match_object.mapMatchedBoundingBox
            .referencePointPositions[size_t(reference_point)];
    auto const para_points =
        ::ad::map::match::getParaPoints(reference_position);
    all_lane_matches.insert(all_lane_matches.end(), para_points.begin(),
                            para_points.end());
  }
  ...
  while ((!_routing_targets.empty()) && (!routing_target_check_finished)) {
    ...
    auto const &distance_to_next_target = ::ad::map::point::distance(
        next_target, carla_rss_state.ego_match_object.enuPosition.centerPoint);
    if (distance_to_next_target < ::ad::physics::Distance(3.)) {
      _routing_targets.erase(_routing_targets.begin());
      _logger->debug("Next target reached: {}; remaining targets: {}",
                     next_target, _routing_targets);
    } else {
  ...
  if (!_routing_targets_to_append.empty()) {
    ...
    _routing_targets.insert(_routing_targets.end(),
                            _routing_targets_to_append.begin(),
                            _routing_targets_to_append.end());
    _logger->debug("Appending new routing targets: {}; resulting targets: {}",
                   _routing_targets_to_append, _routing_targets);
    _routing_targets_to_append.clear();
  }
  ...
  if ((!reroute_required) &&
      (shorten_route_result ==
       ::ad::map::route::ShortenRouteResult::Succeeded)) {
    std::vector<::ad::map::route::FullRoute> additional_routes;
    auto const route_valid = ::ad::map::route::extendRouteToDistance(
        carla_rss_state.ego_route, route_target_length, additional_routes);

    if (route_valid) {
      if (additional_routes.size() > 0u) {
        // take a random extension to the route
        std::size_t route_index = static_cast<std::size_t>(std::rand()) %
                                  (additional_routes.size() + 1);
        if (route_index < additional_routes.size()) {
          // we decided for one of the additional routes
          _logger->debug("Additional Routes: {}->{}", additional_routes.size(),
                         route_index);
  ...
  if (reroute_required) {
    ...
    for (const auto &position :
      ...
      if (!::ad::map::lane::isHeadingInLaneDirection(
        ...
        if (::ad::map::lane::projectPositionToLaneInHeadingDirection(
                start_point,
                carla_rss_state.ego_match_object.enuPosition.heading,
                projected_start_point)) {
          _logger->debug("Projected to lane {}", projected_start_point.laneId);
        }
      }
      _logger->debug("Route start_point: {}, projected_start_point: {}",
                     start_point, projected_start_point);
      auto routing_start_point = ::ad::map::route::planning::createRoutingPoint(
          projected_start_point,
          carla_rss_state.ego_match_object.enuPosition.heading);
      if (!_routing_targets.empty() &&
          ::ad::map::point::isValid(_routing_targets)) {
        auto new_route = ::ad::map::route::planning::planRoute(
            routing_start_point, _routing_targets,
        ...
        auto new_routes = ::ad::map::route::planning::predictRoutesOnDistance(
            routing_start_point, route_target_length,
            ::ad::map::route::RouteCreationMode::AllRoutableLanes);
    ...
    if (!all_new_routes.empty()) {
      ...
      std::size_t route_index =
          static_cast<std::size_t>(std::rand()) % (all_new_routes.size());
...
EgoDynamicsOnRoute RssCheck::CalculateEgoDynamicsOnRoute(
  ...
  new_dynamics.time_since_epoch_check_start_ms =
  ...
  auto object_route = ::ad::map::route::getRouteSection(
      match_object, route,
  ...
  new_dynamics.route_heading = ::ad::map::lane::getENUHeading(
      border, match_object.enuPosition.centerPoint);

  auto const object_center =
      ::ad::map::route::findCenterWaypoint(match_object, object_route);
  if (object_center.isValid()) {
    auto lane_center_point = object_center.queryPosition;
    auto lane_center_point_enu =
        ::ad::map::lane::getENULanePoint(lane_center_point);
    if (std::fabs(new_dynamics.route_heading) >
        ::ad::map::point::ENUHeading(M_PI)) {
      ...
      new_dynamics.route_heading =
          ::ad::map::lane::getENUHeading(border, lane_center_point_enu);
    }
    ...
    if (object_center.laneSegmentIterator->laneInterval.wrongWay) {
      ...
      ::ad::map::lane::projectPositionToLaneInHeadingDirection(
          lane_center_point, new_dynamics.route_heading, lane_center_point);
      lane_center_point_enu =
          ::ad::map::lane::getENULanePoint(lane_center_point);
    }
  ...
  if (last_dynamics.timestamp.elapsed_seconds > 0.) {
    ::ad::physics::Duration const delta_time(
        current_timestamp.elapsed_seconds -
        last_dynamics.timestamp.elapsed_seconds);
    if (delta_time > ::ad::physics::Duration(0.0001)) {
      try {
        new_dynamics.route_accel_lat =
            (new_dynamics.route_speed_lat - last_dynamics.route_speed_lat) /
            delta_time;
        new_dynamics.avg_route_accel_lat =
            ((last_dynamics.avg_route_accel_lat * 2.) +
             new_dynamics.route_accel_lat) /
            3.;
        new_dynamics.route_accel_lon =
            (new_dynamics.route_speed_lon - last_dynamics.route_speed_lon) /
            delta_time;
        new_dynamics.avg_route_accel_lon =
            ((last_dynamics.avg_route_accel_lon * 2.) +
             new_dynamics.route_accel_lon) /
            3.;

        if (new_dynamics.avg_route_accel_lat ==
            ::ad::physics::Acceleration(0.)) {
          // prevent from underrun
          new_dynamics.avg_route_accel_lat = ::ad::physics::Acceleration(0.);
        }
        if (new_dynamics.avg_route_accel_lon ==
            ::ad::physics::Acceleration(0.)) {
          // prevent from underrun
  ...
  if (!object_conversion.calculateMinStoppingDistance(
  ...
  _logger->trace(
      "CalculateEgoDynamicsOnRoute: route-section {} -> dynamics: {}",
      object_route, new_dynamics);
  return new_dynamics;
}
...
::ad::map::landmark::LandmarkIdSet RssCheck::GetGreenTrafficLightsOnRoute(
  ...
  auto next_intersection =
      ::ad::map::intersection::Intersection::getNextIntersectionOnRoute(route);
  if (next_intersection &&
      (next_intersection->intersectionType() ==
    ...
    while (incoming_lanes_iter != incoming_lanes.end()) {
      auto find_waypoint =
          ::ad::map::route::findWaypoint(*incoming_lanes_iter, route);
      if (find_waypoint.isValid() &&
          find_waypoint.laneSegmentIterator->laneInterval.wrongWay) {
    ...
    for (const auto &actor : actors) {
      ...
      carla::geom::BoundingBox trigger_bounding_box =
          traffic_light->GetTriggerVolume();
      ...
      trigger_box_position.x =
          ::ad::map::point::ENUCoordinate(trigger_box_location.x);
      trigger_box_position.y =
          ::ad::map::point::ENUCoordinate(-1 * trigger_box_location.y);
      trigger_box_position.z = ::ad::map::point::ENUCoordinate(0.);

      auto traffic_light_map_matched_positions =
          traffic_light_map_matching.getMapMatchedPositions(
              trigger_box_position, ::ad::physics::Distance(0.25),
              ::ad::physics::Probability(0.1));

      _logger->trace("traffic light map matched positions: {}, {}",
                     trigger_box_position, traffic_light_map_matched_positions);

      for (auto matched_position : traffic_light_map_matched_positions) {
        if (incoming_lanes.find(matched_position.lanePoint.paraPoint.laneId) !=
            incoming_lanes.end()) {
          if (found_relevant_traffic_light &&
              (green_traffic_lights.empty() &&
               (traffic_light_state == carla::rpc::TrafficLightState::Green))) {
            _logger->warn(
                "found another relevant traffic light on lane {}; {} state {}",
                matched_position.lanePoint.paraPoint.laneId,
                traffic_light->GetId(),
                (traffic_light_state == carla::rpc::TrafficLightState::Green)
                    ? "green"
                    : "not green");
...
RssCheck::RssObjectChecker::RssObjectChecker(
    RssCheck const &rss_check, ::ad::rss::map::RssSceneCreation &scene_creation,
...
void RssCheck::RssObjectChecker::operator()(
    const carla::SharedPtr<carla::client::Actor> actor) const {
  const auto vehicle =
      boost::dynamic_pointer_cast<carla::client::Vehicle>(actor);
  if (vehicle == nullptr) {
    return;
  }
  if (vehicle->GetId() == _carla_ego_vehicle.GetId()) {
    return;
  }

  auto const relevant_distance = std::max(
      static_cast<double>(
          _carla_rss_state.ego_dynamics_on_route.min_stopping_distance),
      100.);
  if (vehicle->GetTransform().location.Distance(
          _carla_ego_vehicle.GetTransform().location) > relevant_distance) {
    return;
  }
  ...
  try {
    ...
    auto other_match_object =
        _rss_check.GetMatchObject(*vehicle, ::ad::physics::Distance(2.0));

    _rss_check._logger->debug("OtherVehicleMapMatching: {}",
                              other_match_object.mapMatchedBoundingBox);

    _scene_creation.appendScenes(
        _carla_ego_vehicle.GetId(), _carla_rss_state.ego_match_object,
        _carla_rss_state.ego_dynamics_on_route.ego_speed, ego_dynamics,
        _carla_rss_state.ego_route,
        ::ad::rss::world::ObjectId(vehicle->GetId()),
        ::ad::rss::world::ObjectType::OtherVehicle, other_match_object,
        other_speed, other_dynamics,
        ::ad::rss::map::RssSceneCreation::RestrictSpeedLimitMode::
            IncreasedSpeedLimit10,
        _green_traffic_lights);
  } catch (...) {
    _rss_check._logger->error("Exception processing vehicle {} -> Ignoring it",
                              vehicle->GetId());
  }
}

void RssCheck::CreateWorldModel(carla::client::Timestamp const &timestamp,
                                carla::client::ActorList const &actors,
                                carla::client::Vehicle const &carla_ego_vehicle,
                                CarlaRssState &carla_rss_state) const {
  ::ad::map::landmark::LandmarkIdSet green_traffic_lights =
      GetGreenTrafficLightsOnRoute(actors, carla_rss_state.ego_route);

  auto ego_vehicle_dynamics = GetEgoVehicleDynamics();
  auto const abs_avg_route_accel_lat =
      std::fabs(carla_rss_state.ego_dynamics_on_route.avg_route_accel_lat);
  ...
  ::ad::rss::map::RssSceneCreation scene_creation(timestamp.frame,
                                                  ego_vehicle_dynamics);

  tbb::parallel_for_each(
      actors.begin(), actors.end(),
      RssObjectChecker(*this, scene_creation, carla_ego_vehicle,
                       carla_rss_state, green_traffic_lights));
  ...
  if (_road_boundaries_mode != RoadBoundariesMode::Off) {
    ...
    scene_creation.appendRoadBoundaries(
        carla_ego_vehicle.GetId(), carla_rss_state.ego_match_object,
...
void RssCheck::PerformCheck(CarlaRssState &carla_rss_state) const {
  bool result = carla_rss_state.rss_check.calculateAccelerationRestriction(
      carla_rss_state.world_model, carla_rss_state.situation_snapshot,
      carla_rss_state.rss_state_snapshot, carla_rss_state.proper_response,
      carla_rss_state.acceleration_restriction);

  if (!result) {
    _logger->warn("calculateAccelerationRestriction failed!");
    carla_rss_state.proper_response.lateralResponseRight =
        ::ad::rss::state::LateralResponse::None;
    carla_rss_state.proper_response.lateralResponseLeft =
        ::ad::rss::state::LateralResponse::None;
    carla_rss_state.proper_response.longitudinalResponse =
...
void RssCheck::AnalyseCheckResults(CarlaRssState &carla_rss_state) const {
  ...
  for (auto const state :
       carla_rss_state.rss_state_snapshot.individualResponses) {
    if (::ad::rss::state::isDangerous(state)) {
      carla_rss_state.dangerous_state = true;
      _logger->trace("DangerousState: {}", state);
      auto dangerous_sitation_iter = std::find_if(
          carla_rss_state.situation_snapshot.situations.begin(),
  ...
  if (!vehicle_triggered_longitudinal_response &&
    ...
    carla_rss_state.proper_response.longitudinalResponse =
        ::ad::rss::state::LongitudinalResponse::None;
    carla_rss_state.acceleration_restriction.longitudinalRange.maximum =
        GetEgoVehicleDynamics().alphaLon.accelMax;
  }
  if (!vehicle_triggered_left_response && !left_border_is_dangerous &&
    ...
    carla_rss_state.proper_response.lateralResponseLeft =
        ::ad::rss::state::LateralResponse::None;
    carla_rss_state.acceleration_restriction.lateralLeftRange.maximum =
        GetEgoVehicleDynamics().alphaLat.accelMax;
  ...
  if (!vehicle_triggered_right_response && !right_border_is_dangerous &&
    ...
    carla_rss_state.proper_response.lateralResponseRight =
        ::ad::rss::state::LateralResponse::None;
    carla_rss_state.acceleration_restriction.lateralRightRange.maximum =
        GetEgoVehicleDynamics().alphaLat.accelMax;
    carla_rss_state.ego_dynamics_on_route.crossing_border = true;
  }

  _logger->debug("RouteResponse: {}, AccelerationRestriction: {}",
...
void RssCheck::VisualizeResults(
  ...
  if (_visualization_mutex.try_lock()) {
    ...
    if ((_visualization_mode == VisualizationMode::RouteOnly) ||
        ...
        (_visualization_mode == VisualizationMode::All)) {
      VisualizeRouteLocked(dh, _visualization_route.first, carla_ego_actor,
                           _visualization_route.second);
    }

    if ((_visualization_mode == VisualizationMode::VehicleStateOnly) ||
        (_visualization_mode == VisualizationMode::VehicleStateAndRoute) ||
        (_visualization_mode == VisualizationMode::All)) {
      VisualizeRssResultsLocked(dh, world, carla_ego_actor,
                                _visualization_state_snapshot);
    }

    if ((_visualization_mode == VisualizationMode::All) ||
        (_visualization_mode ==
         VisualizationMode::DebugRouteOrientationObjectRoute) ||
        (_visualization_mode ==
         VisualizationMode::DebugRouteOrientationBorders) ||
    ...
    if ((_visualization_mode ==
         VisualizationMode::DebugRouteOrientationObjectRoute) ||
        (_visualization_mode == VisualizationMode::DebugRouteOrientationBoth)) {
      VisualizeRouteLocked(dh, _visualization_debug_route, carla_ego_actor,
                           false);
    }
    ...
    if ((_visualization_mode ==
      ...
      for (auto const &border : _visualization_debug_enu_border) {
        VisualizeENUEdgeLocked(dh, border.left, color_left,
                               carla_ego_actor->GetLocation().z);
        VisualizeENUEdgeLocked(dh, border.right, color_right,
                               carla_ego_actor->GetLocation().z);
      }
    }
...
void RssCheck::StoreVisualizationResults(CarlaRssState const &carla_rss_state) {
  if (_visualization_mutex.try_lock()) {
    ...
    _visualization_route = std::make_pair(carla_rss_state.ego_route,
                                          carla_rss_state.dangerous_state);
    _visualization_ego_dynamics = carla_rss_state.ego_dynamics_on_route;
    _visualization_mutex.unlock();
  }
}

void RssCheck::StoreDebugVisualization(
...
void RssCheck::VisualizeRssResultsLocked(
  ...
  for (auto const &state : state_snapshot.individualResponses) {
    carla::rpc::ActorId vehicle_id =
        static_cast<carla::rpc::ActorId>(state.objectId);
    ...
    for (const auto &actor : *vehicle_list) {
      ...
      if (_visualization_mode == VisualizationMode::All) {
        ...
        dh.DrawLine(ego_point - line_offset, point - line_offset, 0.1f,
                    lat_l_color, 0.02f, false);
        dh.DrawLine(ego_point + line_offset, point + line_offset, 0.1f,
                    lat_r_color, 0.02f, false);
      }
      point.z += 3.f;
...
void RssCheck::VisualizeENUEdgeLocked(carla::client::DebugHelper &dh,
                                      ::ad::map::point::ENUEdge const &edge,
                                      carla::sensor::data::Color const &color,
                                      float const z_offset) const {
  for (auto const &point : edge) {
    carla::geom::Location carla_point(
        static_cast<float>(static_cast<double>(point.x)),
...
void RssCheck::VisualizeRouteLocked(
    carla::client::DebugHelper &dh, const ::ad::map::route::FullRoute &route,
    carla::SharedPtr<carla::client::Actor> const &carla_ego_actor,
  ...
  for (auto const &road_segment : route.roadSegments) {
    {
      ...
      if (right_lane_edges.find(right_most_lane.laneInterval.laneId) ==
          right_lane_edges.end()) {
        ::ad::map::point::ENUEdge edge =
        ...
        bool intersection_lane =
            ::ad::map::intersection::Intersection::isLanePartOfAnIntersection(
    ...
    }
    {
      ...
      if (left_lane_edges.find(left_most_lane.laneInterval.laneId) ==
          left_lane_edges.end()) {
        ::ad::map::point::ENUEdge edge =
...
void RssCheck::VisualizeEgoDynamics(
  ...
  auto sin_heading = static_cast<float>(
      std::sin(static_cast<double>(ego_dynamics_on_route.route_heading)));
  auto cos_heading = static_cast<float>(
      std::cos(static_cast<double>(ego_dynamics_on_route.route_heading)));
  ...
  dh.DrawArrow(heading_location_start, heading_location_end, 0.1f, 0.1f, color,
               0.02f, false);

  auto sin_center = static_cast<float>(std::sin(
      static_cast<double>(ego_dynamics_on_route.route_heading) + M_PI_2));
  auto cos_center = static_cast<float>(std::cos(
      static_cast<double>(ego_dynamics_on_route.route_heading) + M_PI_2));
  ...
  dh.DrawLine(center_location_start, center_location_end, 0.1f, color, 0.02f,
              false);
}
