...
inline float calculateAngleDelta(float angle1, float angle2) {
...
bool RssCheck::checkObjects(carla::client::Timestamp const &timestamp, carla::client::World &world,
                            ...
                            ::ad_rss::state::ProperResponse &response, bool visualizeResults) {
  ...
  worldModel.timeIndex = timestamp.frame_count;

  ::ad_rss::world::Object egoVehicle;
  ::ad_rss::world::RssDynamics egoDynamics;
  initVehicle(egoVehicle, egoDynamics);
  ...
  if (egoClientWaypoint != nullptr) {
    ...
    for (auto &laneSection : egoRoad->GetLaneSections()) {
      for (const auto &pair : laneSection.GetLanes()) {
        ...
        if ((static_cast<uint32_t>(lane.GetType()) & static_cast<uint32_t>(carla::road::Lane::LaneType::Driving)) > 0) {
          ...
          // lane segment is assumed to be strait and evenly wide, so minimum and maximum length and width
      ...
      std::sort(
          ...
          [&drivingInRoadDirection](::ad_rss::world::LaneSegment const &f, ::ad_rss::world::LaneSegment const &s) {
    ...
    calculateOccupiedRegions(egoBounds, egoVehicle.occupiedRegions, map, drivingInRoadDirection, egoVehicleTransform);
    ...
    for (const auto &actor : *vehicles) {
      ...
      initVehicle(otherVehicle, otherDynamics);
...
void RssCheck::initVehicle(::ad_rss::world::Object &vehicle, ::ad_rss::world::RssDynamics &vehicleDynamics) const {
  vehicle.velocity.speedLon = ::ad_rss::physics::Speed(0.);
  vehicle.velocity.speedLat = ::ad_rss::physics::Speed(0.);
  vehicleDynamics.alphaLon.accelMax = ::ad_rss::physics::Acceleration(3.5);
  vehicleDynamics.alphaLon.brakeMax = ::ad_rss::physics::Acceleration(8.);
  vehicleDynamics.alphaLon.brakeMin = ::ad_rss::physics::Acceleration(4.);
  vehicleDynamics.alphaLon.brakeMinCorrect = ::ad_rss::physics::Acceleration(3);
  vehicleDynamics.alphaLat.accelMax = ::ad_rss::physics::Acceleration(0.2);
  vehicleDynamics.alphaLat.brakeMin = ::ad_rss::physics::Acceleration(0.8);
  vehicleDynamics.responseTime = ::ad_rss::physics::Duration(1.);
}
...
std::array<carla::geom::Location, 4u> RssCheck::getVehicleBounds(const carla::client::Vehicle &vehicle) const {
...
void RssCheck::calculateOccupiedRegions(const std::array<carla::geom::Location, 4u> &bounds,
  ...
  for (const auto &bound : bounds) {
    ...
    auto regionIt = std::find_if(
        ...
        [laneSegmentId](const ::ad_rss::world::OccupiedRegion &region) { return region.segmentId == laneSegmentId; });
...
void RssCheck::visualizeRssResults(::ad_rss::state::RssStateSnapshot stateSnapshot,
  ...
  for (std::size_t i = 0; i < stateSnapshot.individualResponses.size(); ++i) {
    ...
    for (const auto &actor : *vehicleList) {
      ...
      dh.DrawLine(egoPoint, point, 0.1f, lonColor, 0.f, false);
      dh.DrawLine(egoPoint - lineOffset, point - lineOffset, 0.1f, latLColor, 0.f, false);
      dh.DrawLine(egoPoint + lineOffset, point + lineOffset, 0.1f, latRColor, 0.f, false);
      point.z += 3.f;
      dh.DrawPoint(point, 0.2f, indicatorColor, 0.f, false);
    }
  }
}
