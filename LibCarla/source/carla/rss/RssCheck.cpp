...
void printRoute(std::string const &routeDescr, ::ad::map::route::FullRoute const &route) {
  std::cout << routeDescr << std::endl;
  for ( auto roadSegment: route.roadSegments) {
    for (auto laneSegment: roadSegment.drivableLaneSegments) {
      std::cout << "(" << static_cast<uint64_t>(laneSegment.laneInterval.laneId) << " | " << std::setprecision(2) <<  static_cast<double>(laneSegment.laneInterval.start) << ":" << static_cast<double>(laneSegment.laneInterval.end) << ")    ";
    }
    std::cout << std::endl;
  }
}
...
void RssCheck::AppendRoutingTarget(::carla::geom::Transform const &routing_target) {
  _routing_targets_to_append.push_back(::ad::map::point::createENUPoint(routing_target.location.x, -1. * routing_target.location.y, 0.));
}
...
bool RssCheck::CheckObjects(carla::client::Timestamp const &timestamp,
  ...
  try {
    double const time_since_epoch_check_start_ms = std::chrono::duration<double, std::milli>(std::chrono::system_clock::now().time_since_epoch()).count();
#if DEBUG_TIMING
    std::cout << "--- time: " << timestamp.frame << ", " << timestamp.elapsed_seconds << std::endl;
    auto tStart = std::chrono::high_resolution_clock::now();
    auto tEnd =  std::chrono::high_resolution_clock::now();
    std::cout << "-> SC " << std::chrono::duration<double, std::milli>(tEnd-tStart).count() << " start checkObjects" << std::endl;
#endif
    ...
    std::cout << "-> ME " << std::chrono::duration<double, std::milli>(tEnd-tStart).count() << " before  MapMatching" << std::endl;
#endif

    // allow the vehicle to be at least 2.0 m away form the route to not lose the contact to the route
    ...
    if ( ::ad::map::point::isValid(_carla_rss_state.ego_match_object.enuPosition.centerPoint, false) )
    {
      ...
      auto const travelled_distance = ::ad::map::point::distance(_carla_rss_state.ego_match_object.enuPosition.centerPoint, ego_match_object.enuPosition.centerPoint);
      if ( travelled_distance > ::ad::physics::Distance(10.) )
      {
        _logger->warn("Jump in ego vehicle position detected {} -> {}! Force reroute!", _carla_rss_state.ego_match_object.enuPosition.centerPoint, ego_match_object.enuPosition.centerPoint);
        DropRoute();
      }
    }
...
#if DEBUG_TIMING
    tEnd =  std::chrono::high_resolution_clock::now();
    std::cout << "-> ME " << std::chrono::duration<double, std::milli>(tEnd-tStart).count() << " after ego MapMatching" << std::endl;
#endif
...
#if DEBUG_TIMING
    tEnd =  std::chrono::high_resolution_clock::now();
    std::cout << "-> RU " << std::chrono::duration<double, std::milli>(tEnd-tStart).count() << " after route update " << std::endl;
#endif

    _carla_rss_state.ego_dynamics_on_route = CalculateEgoDynamicsOnRoute(timestamp,
                                                                         time_since_epoch_check_start_ms,
                                                                         *carla_ego_vehicle,
                                                                         _carla_rss_state.ego_match_object,
...
#if DEBUG_TIMING
    tEnd =  std::chrono::high_resolution_clock::now();
    std::cout << "-> WM " << std::chrono::duration<double, std::milli>(tEnd-tStart).count() << " after create world model " << std::endl;
#endif
...
#if DEBUG_TIMING
    tEnd =  std::chrono::high_resolution_clock::now();
    std::cout << "-> CH " << std::chrono::duration<double, std::milli>(tEnd-tStart).count() << " end RSS check" << std::endl;
#endif
...
#if DEBUG_TIMING
    tEnd =  std::chrono::high_resolution_clock::now();
    std::cout << "-> VI " << std::chrono::duration<double, std::milli>(tEnd-tStart).count() << " start store viz" << std::endl;
#endif
...
#if DEBUG_TIMING
    tEnd =  std::chrono::high_resolution_clock::now();
    std::cout << "-> VI " << std::chrono::duration<double, std::milli>(tEnd-tStart).count() << " end store viz" << std::endl;
#endif

    _carla_rss_state.ego_dynamics_on_route.time_since_epoch_check_end_ms = std::chrono::duration<double, std::milli>(std::chrono::system_clock::now().time_since_epoch()).count();
...
#if DEBUG_TIMING
    tEnd =  std::chrono::high_resolution_clock::now();
    std::cout << "-> EC " << std::chrono::duration<double, std::milli>(tEnd-tStart).count() << " end check objects" << std::endl;
#endif
...
::ad::map::match::Object RssCheck::GetMatchObject(carla::client::Vehicle const &carla_vehicle, ::ad::physics::Distance const &match_distance) const {
  ...
  match_object.enuPosition.heading = ::ad::map::point::createENUHeading((-1 * vehicle_transform.rotation.yaw) * toRadians);
  ...
  match_object.mapMatchedBoundingBox = map_matching.getMapMatchedBoundingBox(match_object.enuPosition, match_distance, ::ad::physics::Distance(2.));
...
void RssCheck::UpdateRoute(CarlaRssState &carla_rss_state) {
  ...
  // remove the parts of the route already taken, try to prepend route sections (i.e. when driving backwards)
  // try to ensure that the back of the vehicle is still within the route to support orientation calculation
  ::ad::map::point::ParaPointList all_lane_matches;
  for (auto reference_point: {::ad::map::match::ObjectReferencePoints::RearRight, ::ad::map::match::ObjectReferencePoints::RearLeft}) {
    auto const &reference_position = carla_rss_state.ego_match_object.mapMatchedBoundingBox.referencePointPositions[size_t(reference_point)];
  ...
  auto shorten_route_result = ::ad::map::route::shortenRoute(all_lane_matches, carla_rss_state.ego_route,
  ...
  while ((!_routing_targets.empty()) && (!routing_target_check_finished)) {
    ...
    auto const &distance_to_next_target = ::ad::map::point::distance(next_target, carla_rss_state.ego_match_object.enuPosition.centerPoint);
  ...
  if ( !_routing_targets_to_append.empty() ) {
    ...
    _routing_targets.insert(_routing_targets.end(), _routing_targets_to_append.begin(), _routing_targets_to_append.end());
    _logger->debug("Appending new routing targets: {}; resulting targets: {}", _routing_targets_to_append, _routing_targets);
    _routing_targets_to_append.clear();
  }
  ...
  if ((!reroute_required) && (shorten_route_result == ::ad::map::route::ShortenRouteResult::Succeeded)) {
    ...
    auto const route_valid = ::ad::map::route::extendRouteToDistance(carla_rss_state.ego_route, route_target_length,
                                                                     additional_routes);
  ...
  if (reroute_required) {
    ...
    for (const auto &position : carla_rss_state.ego_match_object.mapMatchedBoundingBox.referencePointPositions[int32_t(::ad::map::match::ObjectReferencePoints::Center)]) {
      auto start_point = position.lanePoint.paraPoint;
      auto projected_start_point = start_point;
      if (!::ad::map::lane::isHeadingInLaneDirection(start_point, carla_rss_state.ego_match_object.enuPosition.heading)) {
      ...
      auto routing_start_point = ::ad::map::route::planning::createRoutingPoint(projected_start_point, carla_rss_state.ego_match_object.enuPosition.heading);
                                                         ...
                                                         EgoDynamicsOnRoute const &last_dynamics) const
{
  ...
  auto object_route = ::ad::map::route::getRouteSection(match_object, route, ::ad::map::route::RouteSectionCreationMode::AllRouteLanes);
  ...
  if (object_center.isValid()) {
    ...
    {
      // if the actual center point is already outside, try to use this extended object center for the route heading calculation
    ...
    if (object_center.laneSegmentIterator->laneInterval.wrongWay) {
      ...
      ::ad::map::lane::projectPositionToLaneInHeadingDirection(lane_center_point, new_dynamics.route_heading, lane_center_point);
    ...
    // the ego vehicle is completely outside the route, so we can't update the values
  ...
  new_dynamics.route_speed_lon = std::fabs(std::cos(static_cast<double>(new_dynamics.heading_diff))) * new_dynamics.ego_speed;
  ...
  if ( last_dynamics.timestamp.elapsed_seconds > 0. ) {
    ::ad::physics::Duration const delta_time(current_timestamp.elapsed_seconds - last_dynamics.timestamp.elapsed_seconds);
    if ( delta_time > ::ad::physics::Duration(0.0001) ) {
      try {
        ...
        new_dynamics.avg_route_accel_lat = ((last_dynamics.avg_route_accel_lat * 2.) + new_dynamics.route_accel_lat)/3.;
        new_dynamics.route_accel_lon = (new_dynamics.route_speed_lon - last_dynamics.route_speed_lon)/delta_time;
        new_dynamics.avg_route_accel_lon = ((last_dynamics.avg_route_accel_lon * 2.) + new_dynamics.route_accel_lon)/3.;
  ...
  // check if the center point (and only the center point) is still found on the route
  ::ad::map::point::ParaPointList in_lane_matches;
  for (auto &match_position: match_object.mapMatchedBoundingBox.referencePointPositions[int32_t(::ad::map::match::ObjectReferencePoints::Center)]) {
  ...
  {
    _logger->error("CalculateEgoDynamicsOnRoute: calculation of min stopping distance failed. Setting to 100. ({} {} {})",
...
::ad::map::landmark::LandmarkIdSet RssCheck::GetGreenTrafficLightsOnRoute(carla::client::ActorList const &actors,
                                                                          ::ad::map::route::FullRoute const&route) const
{
  ...
  if ( next_intersection && (next_intersection->intersectionType() ==  ::ad::map::intersection::IntersectionType::TrafficLight) ) {
    // try to guess the the relevant traffic light with the rule: nearest traffic light in respect to the incoming lane.
    // @todo: when OpenDrive maps have the traffic lights incorporated, we only have to fill all green traffic lights into the green_traffic_lights list
    auto incoming_lanes = next_intersection->incomingLanesOnRoute();
    // since our route spans the whole street, we have to filter out the incoming lanes with wrong way flag
    ...
    for (const auto &actor : actors) {
      ...
      auto traffic_light_map_matched_positions = traffic_light_map_matching.getMapMatchedPositions(trigger_box_position, ::ad::physics::Distance(0.25), ::ad::physics::Probability(0.1));

      _logger->trace("traffic light map matched positions: {}, {}", trigger_box_position, traffic_light_map_matched_positions);
      ...
      for (auto matched_position: traffic_light_map_matched_positions)
      {
        ...
        {
          if ( found_relevant_traffic_light && (green_traffic_lights.empty() && (traffic_light_state==carla::rpc::TrafficLightState::Green)) )
          {
          ...
          {
            // @todo: currently there is only this workaround because of missign OpenDrive map support for actual traffic light ids
...
RssCheck::RssObjectChecker::RssObjectChecker(RssCheck const& rss_check, ::ad::rss::map::RssSceneCreation &scene_creation, carla::client::Vehicle const &carla_ego_vehicle, CarlaRssState const &carla_rss_state,
...
void RssCheck::RssObjectChecker::operator()( const  carla::SharedPtr<carla::client::Actor> actor ) const {
  ...
  auto const relevant_distance = std::max(static_cast<double>(_carla_rss_state.ego_dynamics_on_route.min_stopping_distance), 100.);
  if (vehicle->GetTransform().location.Distance(_carla_ego_vehicle.GetTransform().location) > relevant_distance) {
    return;
  }
  ...
  try {
    ...
    _scene_creation.appendScenes(_carla_ego_vehicle.GetId(),
                                 _carla_rss_state.ego_match_object,
                                 _carla_rss_state.ego_dynamics_on_route.ego_speed,
                                 ego_dynamics,
                                 _carla_rss_state.ego_route,
...
void RssCheck::CreateWorldModel(carla::client::Timestamp const &timestamp,
  ...
  ::ad::map::landmark::LandmarkIdSet green_traffic_lights = GetGreenTrafficLightsOnRoute(actors, carla_rss_state.ego_route);
  ...
  {
    _logger->info("!! Route lateral dynamics exceed expectations: route:{} expected:{} !!",
                  abs_avg_route_accel_lat,
  ...
  tbb::parallel_for_each( actors.begin(), actors.end(), RssObjectChecker(*this, scene_creation, carla_ego_vehicle, carla_rss_state, green_traffic_lights) );
  ...
  if ( _road_boundaries_mode != RoadBoundariesMode::Off ) {
    ...
    scene_creation.appendRoadBoundaries(carla_ego_vehicle.GetId(),
                                        carla_rss_state.ego_match_object,
                                        carla_rss_state.ego_dynamics_on_route.ego_speed,
                                        ego_vehicle_dynamics,
                                        carla_rss_state.ego_route,
                                        // since the route always expanded, route isn't required to expand any more
...
{
  bool result = carla_rss_state.rss_check.calculateAccelerationRestriction(carla_rss_state.world_model,
                                                                           carla_rss_state.situation_snapshot,
                                                                           carla_rss_state.rss_state_snapshot,
...
void RssCheck::AnalyseCheckResults(CarlaRssState &carla_rss_state) const {
  ...
  _logger->debug("RouteResponse: {}, AccelerationRestriction: {}", carla_rss_state.proper_response, carla_rss_state.acceleration_restriction);
}
...
void RssCheck::VisualizeRssResultsLocked(carla::client::DebugHelper &dh,
...
void RssCheck::VisualizeRouteLocked(carla::client::DebugHelper &dh,
                                    const ::ad::map::route::FullRoute &route,
                                    carla::SharedPtr<carla::client::Actor> const &carla_ego_actor,
