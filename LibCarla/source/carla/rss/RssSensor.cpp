...
#include "carla/rss/EgoVehicleData.h"
...
SharedPtr<sensor::SensorData> RssSensor::TickRssSensor(const Timestamp &timestamp) {
  try {
    ...
    if (result) {
      ...
      return MakeShared<sensor::data::RssResponse>(timestamp.frame_count, timestamp.elapsed_seconds,
                                                        GetTransform(), longitudinalResponse, lateralResponseRight,
                                                        lateralResponseLeft);
    } else {
      return nullptr;
    }
