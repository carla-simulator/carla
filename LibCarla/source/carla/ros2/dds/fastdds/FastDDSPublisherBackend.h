...
/// Fast DDS implementation of IDDSPublisherBackend.
...
template<typename T>
class FastDDSPublisherBackend
    : public IDDSPublisherBackend
...
public:
  ...
  ~FastDDSPublisherBackend() override {
    if (_datawriter)
  ...
  bool Init(const std::string& topic_name) override {
    if (_type == nullptr) {
      log_error("FastDDSPublisherBackend: Invalid TypeSupport");
      return false;
    }
    ...
    if (_participant == nullptr) {
      log_error("FastDDSPublisherBackend: Failed to create DomainParticipant");
      return false;
    }
    ...
    if (_publisher == nullptr) {
      log_error("FastDDSPublisherBackend: Failed to create Publisher");
      return false;
    }
    ...
    if (_topic == nullptr) {
      log_error("FastDDSPublisherBackend: Failed to create Topic");
      return false;
    }
    ...
    if (_datawriter == nullptr) {
      log_error("FastDDSPublisherBackend: Failed to create DataWriter");
      return false;
    }
  ...
  bool Publish(void* message_data) override {
    ...
    to_fastdds(*pod, _fastdds_msg);
    eprosima::fastrtps::rtps::InstanceHandle_t instance_handle;
    erc rcode = _datawriter->write(&_fastdds_msg, instance_handle);
    if (rcode == erc::ReturnCodeValue::RETCODE_OK) {
      return true;
    } else {
      log_error("FastDDSPublisherBackend::Publish (", _topic_name, ") failed with code:", rcode());
      return false;
    }
  }
