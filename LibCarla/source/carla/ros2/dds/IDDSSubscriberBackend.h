...
/// Type-erased abstract interface for a DDS subscriber backend.
...
class IDDSSubscriberBackend {
public:
  virtual ~IDDSSubscriberBackend() = default;

  /// Initialize DDS entities (participant, subscriber, topic, reader).
  /// The backend writes incoming messages to *message_ptr and sets *new_message_flag = true.
  ...
  virtual bool Init(const std::string& topic_name, void* message_ptr, bool* new_message_flag) = 0;
