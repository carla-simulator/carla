...
std::pair<std::shared_ptr<CarlaPublisher>, std::shared_ptr<CarlaTransformPublisher>> ROS2::GetOrCreateSensor(int type, carla::streaming::detail::stream_id_type id, const char* ros_name) {
  ...
  if (it_publishers != _publishers.end()) {
    ...
    switch(type) {
      case ESensors::CollisionSensor: {
        ...
        std::shared_ptr<CarlaCollisionPublisher> new_publisher = std::make_shared<CarlaCollisionPublisher>(pub_name.c_str());
      ...
      case ESensors::DepthCamera: {
        ...
        std::shared_ptr<CarlaDepthCameraPublisher> new_publisher = std::make_shared<CarlaDepthCameraPublisher>(pub_name.c_str());
        if (new_publisher->Init()) {
          _publishers.insert({id, new_publisher});
          publisher = new_publisher;
        }
      ...
      case ESensors::DVSCamera: {
        ...
        std::shared_ptr<CarlaDVSCameraPublisher> new_publisher = std::make_shared<CarlaDVSCameraPublisher>(pub_name.c_str());
        if (new_publisher->Init()) {
          _publishers.insert({id, new_publisher});
          publisher = new_publisher;
        }
      ...
      case ESensors::GnssSensor: {
        ...
        std::shared_ptr<CarlaGNSSPublisher> new_publisher = std::make_shared<CarlaGNSSPublisher>(pub_name.c_str());
        if (new_publisher->Init()) {
          _publishers.insert({id, new_publisher});
          publisher = new_publisher;
        }
      ...
      case ESensors::InertialMeasurementUnit: {
        ...
        std::shared_ptr<CarlaIMUPublisher> new_publisher = std::make_shared<CarlaIMUPublisher>(pub_name.c_str());
        if (new_publisher->Init()) {
          _publishers.insert({id, new_publisher});
          publisher = new_publisher;
        }
      ...
      case ESensors::LaneInvasionSensor: {
        std::string pub_name = ros_name;
        if (pub_name.empty()) {
          pub_name = "lane_nnvasion_";
          pub_name += string_id;
        }
        std::shared_ptr<CarlaLineInvasionPublisher> new_publisher = std::make_shared<CarlaLineInvasionPublisher>(pub_name.c_str());
        if (new_publisher->Init()) {
          _publishers.insert({id, new_publisher});
          publisher = new_publisher;
        }
      ...
      case ESensors::Radar: {
        ...
        std::shared_ptr<CarlaRadarPublisher> new_publisher = std::make_shared<CarlaRadarPublisher>(pub_name.c_str());
        if (new_publisher->Init()) {
          _publishers.insert({id, new_publisher});
          publisher = new_publisher;
        }
      ...
      case ESensors::RayCastSemanticLidar: {
        ...
        std::shared_ptr<CarlaSemanticLidarPublisher> new_publisher = std::make_shared<CarlaSemanticLidarPublisher>(pub_name.c_str());
        if (new_publisher->Init()) {
          _publishers.insert({id, new_publisher});
          publisher = new_publisher;
        }
      ...
      case ESensors::RayCastLidar: {
        ...
        std::shared_ptr<CarlaLidarPublisher> new_publisher = std::make_shared<CarlaLidarPublisher>(pub_name.c_str());
        if (new_publisher->Init()) {
          _publishers.insert({id, new_publisher});
          publisher = new_publisher;
        }
      ...
      case ESensors::SceneCaptureCamera: {
        ...
        std::shared_ptr<CarlaRGBCameraPublisher> new_publisher = std::make_shared<CarlaRGBCameraPublisher>(pub_name.c_str());
        if (new_publisher->Init()) {
          _publishers.insert({id, new_publisher});
          publisher = new_publisher;
        }
      ...
      case ESensors::SemanticSegmentationCamera: {
        ...
        std::shared_ptr<CarlaSSCameraPublisher> new_publisher = std::make_shared<CarlaSSCameraPublisher>(pub_name.c_str());
        if (new_publisher->Init()) {
          _publishers.insert({id, new_publisher});
          publisher = new_publisher;
        }
...
void ROS2::ProcessDataFromSensor(
  ...
  switch (sensor_type) {
    ...
    case ESensors::DepthCamera:
      {
        ...
        auto sensors = GetOrCreateSensor(ESensors::DepthCamera, stream_id, GetActorRosName(actor).c_str());
        if (sensors.first) {
    ...
    case ESensors::SceneCaptureCamera:
    {
      ...
      {
        auto sensors = GetOrCreateSensor(ESensors::SceneCaptureCamera, stream_id, GetActorRosName(actor).c_str());
        if (sensors.first) {
...
void ROS2::ProcessDataFromGNSS(
  ...
  auto sensors = GetOrCreateSensor(ESensors::GnssSensor, stream_id, GetActorRosName(actor).c_str());
  if (sensors.first) {
...
void ROS2::ProcessDataFromIMU(
  ...
  auto sensors = GetOrCreateSensor(ESensors::InertialMeasurementUnit, stream_id, GetActorRosName(actor).c_str());
  if (sensors.first) {
...
void ROS2::ProcessDataFromLidar(
  ...
  auto sensors = GetOrCreateSensor(ESensors::RayCastLidar, stream_id, GetActorRosName(actor).c_str());
  if (sensors.first) {
...
void ROS2::ProcessDataFromSemanticLidar(
  ...
  auto sensors = GetOrCreateSensor(ESensors::RayCastSemanticLidar, stream_id, GetActorRosName(actor).c_str());
  if (sensors.first) {
    std::shared_ptr<CarlaLidarPublisher> publisher = std::dynamic_pointer_cast<CarlaLidarPublisher>(sensors.first);
...
void ROS2::ProcessDataFromRadar(
  ...
  auto sensors = GetOrCreateSensor(ESensors::Radar, stream_id, GetActorRosName(actor).c_str());
  if (sensors.first) {
    std::shared_ptr<CarlaLidarPublisher> publisher = std::dynamic_pointer_cast<CarlaLidarPublisher>(sensors.first);
