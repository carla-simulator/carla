...
namespace ros2 {
  ...
  bool CarlaLidarPublisher::Init() {
    ...
    const std::string topic_name {"rt/carla/lidar"};
...
void CarlaLidarPublisher::SetData(size_t height, size_t width, const uint8_t* data, const char* frame_id) {
    const size_t point_size = 4 * sizeof(float);
    std::vector<uint8_t> vector_data;
    const size_t size = height * 5 * point_size;
    vector_data.resize(size);
    std::memcpy(&vector_data[0], &data[0], size);    
    SetData(height, 5, std::move(vector_data), frame_id);
  }

  void CarlaLidarPublisher::SetData(size_t height, size_t width, std::vector<uint8_t>&& data, const char* frame_id) {
  ...
  _impl(std::make_unique<CarlaLidarPublisherImpl>()) {}
