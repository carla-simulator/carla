...
bool WalkerPublisher::Init(std::shared_ptr<DdsDomainParticipantImpl> domain_participant) {
  return _walker_odometry_publisher->Init(domain_participant, get_topic_name("odometry"), get_topic_qos()) &&
    _walker_object_publisher->Init(domain_participant) &&
    _walker_object_with_covariance_publisher->Init(domain_participant);
}

bool WalkerPublisher::Publish() {
  auto success = _walker_odometry_publisher->Publish();
  success &= _walker_object_publisher->Publish();
  success &= _walker_object_with_covariance_publisher->Publish();
  return success;
}

bool WalkerPublisher::SubscribersConnected() const {
  return _walker_odometry_publisher->SubscribersConnected() ||
    _walker_object_publisher->SubscribersConnected() ||
