...
// Resolve the shipped default config relative to this shared library. The
// build stages zenoh_session_config.json5 next to libcarla-ros2-native.so
// (both in the plugin Binaries/Linux and in the packaged tree), so dladdr on
// a symbol we own gives the directory to look in. This works in both editor
// and packaged runs, unlike __FILE__ (which points at a build-machine source
// path that does not exist after packaging).
std::string default_config_path() {
  Dl_info info;
  if (dladdr(reinterpret_cast<const void*>(&zenoh_get_shared_session), &info) &&
      info.dli_fname != nullptr) {
    const std::string so{info.dli_fname};
    const size_t slash = so.find_last_of('/');
    if (slash != std::string::npos) {
      return so.substr(0, slash) + "/zenoh_session_config.json5";
    }
  }
  // Last resort: look in the current working directory.
  return "zenoh_session_config.json5";
}
